...
var Yc = new RegExp(`(\\:not\\()|(([\\.\\#]?)[-\\w]+)|(?:\\[([-.\\w*\\\\$]+)(?:=(["']?)([^\\]"']*)\\5)?\\])|(\\))|(\\s*,\\s*)`, "g"), Xn = class n {
    ...
    attrs = [];
    notSelectors = [];
    static parse(e) { let t = [], s = (l, c) => { c.notSelectors.length > 0 && !c.element && c.classNames.length == 0 && c.attrs.length == 0 && (c.element = "*"), l.push(c); }, r = new n, i, o = r, a = !1; for (Yc.lastIndex = 0; i = Yc.exec(e);) {
        if (i[1]) {
            if (a)
    ...
    _listContexts = [];
    addSelectables(e, t) { let s = null; e.length > 1 && (s = new Na(e), this._listContexts.push(s)); for (let r = 0; r < e.length; r++)
        this._addSelectable(e[r], t, s); }
    _addSelectable(e, t, s) { let r = this, i = e.element, o = e.classNames, a = e.attrs, l = new Da(e, t, s); if (i && (a.length === 0 && o.length === 0 ? this._addTerminal(r._elementMap, i, l) : r = this._addPartial(r._elementPartialMap, i)), o)
...
function dd(n) { Mi ??= new TextEncoder; let e = [...Mi.encode(n)], t = vd(e, Sc.Big), s = e.length * 8, r = new Uint32Array(80), i = 1732584193, o = 4023233417, a = 2562383102, l = 271733878, c = 3285377520; t[s >> 5] |= 128 << 24 - s % 32, t[(s + 64 >> 9 << 4) + 15] = s; for (let p = 0; p < t.length; p += 16) {
...
function Zc(n, e, t) { let s = 2654435769, r = 2654435769, i = 0, o = e - 12; for (; i <= o; i += 12) {
    s += n.getUint32(i, !0), r += n.getUint32(i + 4, !0), t += n.getUint32(i + 8, !0);
    let l = Jc(s, r, t);
    s = l[0], r = l[1], t = l[2];
} let a = e - i; return t += e, a >= 4 ? (s += n.getUint32(i, !0), i += 4, a >= 8 ? (r += n.getUint32(i, !0), i += 4, a >= 9 && (t += n.getUint8(i++) << 8), a >= 10 && (t += n.getUint8(i++) << 16), a === 11 && (t += n.getUint8(i++) << 24)) : (a >= 5 && (r += n.getUint8(i++)), a >= 6 && (r += n.getUint8(i++) << 8), a === 7 && (r += n.getUint8(i++) << 16))) : (a >= 1 && (s += n.getUint8(i++)), a >= 2 && (s += n.getUint8(i++) << 8), a === 3 && (s += n.getUint8(i++) << 16)), Jc(s, r, t)[2]; }
...
function vd(n, e) { let t = n.length + 3 >>> 2, s = []; for (let r = 0; r < t; r++)
    s[r] = wd(n, r * 4, e); return s; }
function Kc(n, e) { return e >= n.length ? 0 : n[e]; }
function wd(n, e, t) { let s = 0; if (t === Sc.Big)
    for (let r = 0; r < 4; r++)
        s += Kc(n, e + r) << 24 - 8 * r;
else
    for (let r = 0; r < 4; r++)
        s += Kc(n, e + r) << 8 * r; return s; }
...
function nh(n, e, t) { let s = n.length; if (s !== e.length)
...
function nu(n) { let e = ""; if (n.tagName && (e += ` @${n.tagName}`), n.text) {
    if (n.text.match(/\/\*|\*\//))
...
var oA = Object.freeze({ __proto__: null, ArrayType: Ma, ArrowFunctionExpr: En, BOOL_TYPE: eh, get BinaryOperator() { return x; }, BinaryOperatorExpr: H, BuiltinType: Ct, get BuiltinTypeName() { return Ht; }, CommaExpr: Oa, ConditionalExpr: _t, DYNAMIC_TYPE: qe, DeclareFunctionStmt: yn, DeclareVarStmt: le, DynamicImportExpr: Qn, Expression: G, ExpressionStatement: tt, ExpressionType: Ze, ExternalExpr: Jt, ExternalReference: $a, FUNCTION_TYPE: Sd, FnParam: Z, FunctionExpr: Yt, INFERRED_TYPE: Oe, INT_TYPE: Ed, IfStmt: Ar, InstantiateExpr: Is, InvokeFunctionExpr: Me, JSDocComment: Cr, LeadingComment: yr, LiteralArrayExpr: ft, LiteralExpr: Ee, LiteralMapEntry: Zn, LiteralMapExpr: bt, LiteralPiece: dn, LocalizedString: xr, MapType: Ra, NONE_TYPE: At, NULL_EXPR: Sn, NUMBER_TYPE: Zo, NotExpr: Ds, ParenthesizedExpr: Tt, PlaceholderPiece: Un, ReadKeyExpr: Mt, ReadPropExpr: Ue, ReadVarExpr: et, RecursiveAstVisitor: _d, ReturnStatement: _e, STRING_TYPE: Jo, Statement: xn, get StmtModifier() { return ae; }, TYPED_NULL_EXPR: sh, TaggedTemplateLiteralExpr: ks, TemplateLiteralElementExpr: Sr, TemplateLiteralExpr: Ns, TransplantedType: Fi, Type: wn, get TypeModifier() { return xc; }, TypeofExpr: Yn, get UnaryOperator() { return bs; }, UnaryOperatorExpr: Kt, VoidExpr: Er, WrappedNodeExpr: U, areAllEquivalent: Be, arrowFn: ie, expressionType: we, fn: en, ifStmt: pi, importExpr: y, importType: bd, isNull: Nd, jsDocComment: rh, leadingComment: Td, literal: d, literalArr: R, literalMap: pe, localizedString: ah, not: ih, nullSafeIsEquivalent: th, taggedTemplate: oh, transplantedType: kd, typeofExpr: Xs, unary: Id, variable: N }), Pd = "_c", su = N("<unknown>"), Ld = {}, Bd = 50, $i = class n extends G {
    ...
    getConstLiteral(e, t) { if (e instanceof Ee && !ru(e) || e instanceof $i)
        return e; let s = xs.INSTANCE.keyOf(e), r = this.literals.get(s), i = !1; if (r || (r = new $i(e), this.literals.set(s, r), i = !0), !i && !r.shared || i && t) {
    ...
    getLiteralFactory(e) { if (e instanceof ft) {
    ...
    getSharedFunctionReference(e, t, s = !0) { let r = e instanceof En; for (let o of this.statements)
        if (r && o instanceof le && o.value?.isEquivalent(e) || !r && o instanceof yn && e instanceof Yt && e.isEquivalent(o))
            return N(o.name); let i = s ? this.uniqueName(t) : t; return this.statements.push(e instanceof Yt ? e.toDeclStmt(i, ae.Final) : new le(i, e, Oe, ae.Final, e.sourceSpan)), N(i); }
    ...
    keyOf(e) { if (e instanceof Ee && typeof e.value == "string")
        ...
        return String(e.value); if (e instanceof ft) {
        let t = [];
...
function qd(n) { let e = []; for (let t = 0; t < n.length; t++) {
...
var Hd = 3, Wd = "# sourceMappingURL=data:application/json;base64,", qa = class {
    ...
    toJSON() { if (!this.hasMappings)
        return null; let e = new Map, t = [], s = []; Array.from(this.sourcesContent.keys()).forEach((c, p) => { e.set(c, p), t.push(c), s.push(this.sourcesContent.get(c) || null); }); let r = "", i = 0, o = 0, a = 0, l = 0; return this.lines.forEach(c => { i = 0, r += c.map(p => { let h = gi(p.col0 - i); return i = p.col0, p.sourceUrl != null && (h += gi(e.get(p.sourceUrl) - o), o = e.get(p.sourceUrl), h += gi(p.sourceLine0 - a), a = p.sourceLine0, h += gi(p.sourceCol0 - l), l = p.sourceCol0), h; }).join(","), r += ";"; }), r = r.slice(0, -1), { file: this.file || "", version: Hd, sourceRoot: "", sources: t, sourcesContent: s, mappings: r }; }
    toJsComment() { return this.hasMappings ? "//" + Wd + jd(JSON.stringify(this, null, 0)) : ""; }
};
function jd(n) { let e = "", t = qd(n); for (let s = 0; s < t.length;) {
...
function gi(n) { n = n < 0 ? (-n << 1) + 1 : n << 1; let e = ""; do {
    let t = n & 31;
    n = n >> 5, n > 0 && (t = t | 32), e += rr(t);
} while (n > 0); return e; }
var zd = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
function rr(n) { if (n < 0 || n >= 64)
    throw new Error("Can only encode value in the range [0, 63]"); return zd[n]; }
...
`);
    ...
    toSourceMapGenerator(e, t = 0) { let s = new qa(e), r = !1, i = () => { r || (s.addSource(e, " ").addMapping(0, e, 0, 0), r = !0); }; for (let o = 0; o < t; o++)
        s.addLine(), i(); return this.sourceLines.forEach((o, a) => { s.addLine(); let l = o.srcSpans, c = o.parts, p = o.indent * Ua.length, h = 0; for (; h < l.length && !l[h];)
        p += c[h].length, h++; for (h < l.length && a === 0 && p === 0 ? r = !0 : i(); h < l.length;) {
...
`).forEach(r => { t.println(e, `// ${r}`); });
    ...
    visitBinaryOperatorExpr(e, t) { let s = Yd.get(e.operator); if (!s)
...
function iu(n) { let e = ""; for (let t = 0; t < n; t++)
    e += Ua; return e; }
function Ko(n, e) { if (e === 0)
    return we(n); let t = []; for (let s = 0; s < e; s++)
    t.push(qe); return we(n, void 0, t); }
...
function vs({ expression: n, forwardRef: e }) { switch (e) {
    case 0:
    case 1: return n;
    case 2: return Cc(n);
} }
...
function Rn(n) { let e = N("__ngFactoryType__"), t = null, s = au(n) ? e : new H(x.Or, e, n.type.value), r = null; n.deps !== null ? n.deps !== "invalid" && (r = new Is(s, ou(n.deps, n.target))) : (t = N(`\u0275${n.name}_BaseFactory`), r = t.callFn([s])); let i = [], o = null; function a(c) { let p = N("__ngConditionalFactory__"); i.push(new le(p.name, Sn, Oe)); let h = r !== null ? p.set(r).toStmt() : y(f.invalidFactory).callFn([]).toStmt(); return i.push(pi(e, [h], [p.set(c).toStmt()])), p; } if (au(n)) {
    let c = ou(n.delegateDeps, n.target), p = new (n.delegateType === qi.Class ? Is : Me)(n.delegate, c);
    o = a(p);
}
else
    tm(n) ? o = a(n.expression) : o = r; if (o === null)
    ...
    i.push(new _e(o)); let l = en([new Z(e.name, qe)], i, Oe, void 0, `${n.name}_Factory`); return t !== null && (l = ie([], [new le(t.name), new _e(l)]).callFn([], void 0, !0)), { expression: l, statements: [], type: hh(n) }; }
...
function Jd(n, e, t) { if (n.token === null)
...
var ea = class {
    ...
    static fromParsedEvent(e) { let t = e.type === pt.Regular ? e.targetOrPhase : null, s = e.type === pt.LegacyAnimation ? e.targetOrPhase : null; if (e.keySpan === void 0)
...
function j(n, e) { let t = []; if (n.visit)
    for (let s of e)
        n.visit(s);
else
    for (let s of e) {
...
<!ELEMENT ex (#PCDATA)>`, il = class extends $s {
    ...
    visitTagPlaceholder(e, t) { let s = new X(`<${e.tag}>`), r = new M(Mn, {}, [s]), i = new M(Bn, { name: e.startName }, [r, s]); if (e.isVoid)
        return [i]; let o = new X(`</${e.tag}>`), a = new M(Mn, {}, [o]), l = new M(Bn, { name: e.closeName }, [a, o]); return [i, ...this.serialize(e.children), l]; }
...
function fi(n, e = !0) { let t = Tc(n); if (!e)
...
function ll(n, e) { let t = Object.getOwnPropertyNames(n); return t.length === 0 ? null : pe(t.map(s => { let r = n[s], i, o, a, l; if (typeof r == "string")
    i = s, a = s, o = r, l = Le(o);
else {
...
function uu(n, e) { let t = null, s = { name: n.name, type: n.type, typeArgumentCount: n.typeArgumentCount, deps: [], target: Pt.Injectable }; if (n.useClass !== void 0) {
...
var Dm = 0;
function Pm(n) { if (!n || !n.reference)
    return null; let e = n.reference; if (e.__anonymousType)
    return e.__anonymousType; if (e.__forward_ref__)
    return "__forward_ref__"; let t = ch(e); return t.indexOf("(") >= 0 ? (t = `anonymous_${Dm++}`, e.__anonymousType = t) : t = fr(t), t; }
...
var vu = '(this&&this.__makeTemplateObject||function(e,t){return Object.defineProperty?Object.defineProperty(e,"raw",{value:t}):e.raw=t,e})', pl = class extends Wa {
    ...
    visitLocalizedString(e, t) { t.print(e, `$localize(${vu}(`); let s = [e.serializeI18nHead()]; for (let r = 1; r < e.messageParts.length; r++)
        s.push(e.serializeI18nTemplatePart(r)); return t.print(e, `[${s.map(r => $n(r.cooked, !1)).join(", ")}], `), t.print(e, `[${s.map(r => $n(r.raw, !1)).join(", ")}])`), e.expressions.forEach(r => { t.print(e, ", "), r.visitExpression(this, t); }), t.print(e, ")"), null; }
    _visitParams(e, t) { this.visitAllObjects(s => t.print(null, s.name), e, t, ","); }
}, vi;
function Lm() { if (vi === void 0) {
...
function Rm(n) { let e = [], t = new oe; if (t.set("type", n.type.value), n.kind === Gt.Global && n.bootstrap.length > 0 && t.set("bootstrap", xt(n.bootstrap, n.containsForwardDecls)), n.selectorScopeMode === ho.Inline)
    ...
    let i = $m(n);
    i !== null && e.push(i);
} n.schemas !== null && n.schemas.length > 0 && t.set("schemas", R(n.schemas.map(i => i.value))), n.id !== null && (t.set("id", n.id), e.push(y(f.registerNgModuleType).callFn([n.type.value, n.id]).toStmt())); let s = y(f.defineNgModule).callFn([t.toLiteralMap()], void 0, !0), r = Rh(n); return { expression: s, type: r, statements: e }; }
...
function Rh(n) { if (n.kind === Gt.Local)
    return new Ze(n.type.value); let { type: e, declarations: t, exports: s, imports: r, includeImportTypes: i, publicDeclarationTypes: o } = n; return new Ze(y(f.NgModuleDeclaration, [new Ze(e.type), o === null ? la(t) : Om(o), i ? la(r) : At, la(s)])); }
function $m(n) { let e = new oe; if (n.kind === Gt.Global ? n.declarations.length > 0 && e.set("declarations", xt(n.declarations, n.containsForwardDecls)) : n.declarationsExpression && e.set("declarations", n.declarationsExpression), n.kind === Gt.Global ? n.imports.length > 0 && e.set("imports", xt(n.imports, n.containsForwardDecls)) : n.importsExpression && e.set("imports", n.importsExpression), n.kind === Gt.Global ? n.exports.length > 0 && e.set("exports", xt(n.exports, n.containsForwardDecls)) : n.exportsExpression && e.set("exports", n.exportsExpression), n.kind === Gt.Local && n.bootstrapExpression && e.set("bootstrap", n.bootstrapExpression), Object.keys(e.values).length === 0)
    return null; let t = new Me(y(f.setNgModuleScope), [n.type.value, e.toLiteralMap()]), s = Zd(t), r = new Yt([], [s.toStmt()]); return new Me(r, []).toStmt(); }
...
`;
    ...
    _convertColonHost(e) { return e.replace(jm, (t, s, r) => { if (s) {
    ...
    _convertColonHostContextInSelectorPart(e) { return e.replace(zm, (t, s) => { let r = [[]], i; for (; i = Gm.exec(t);) {
    ...
    _applySimpleSelectorScope(e, t, s) { if (Es.lastIndex = 0, Es.test(e)) {
        let r = `[${s}]`, i = e;
        for (; i.match(ca);)
            i = i.replace(ca, (o, a) => a.replace(/([^:\)]*)(:*)(.*)/, (l, c, p, h) => c + r + p + h));
        return i.replace(Es, r);
    } return t + " " + e; }
    _applySelectorScope({ selector: e, scopeSelector: t, hostSelector: s, isParentSelector: r }) { let i = /\[is=([^\]]*)\]/g; t = t.replace(i, (C, ...b) => b[0]); let o = `[${t}]`, a = C => { let b = C.trim(); if (!b)
        return C; if (C.includes(cn)) {
    ...
    } return b; }, l = C => { let b = "", _ = [], P; for (; (P = tr.exec(C)) !== null;) {
...
function lg(n, e, t) { let s = [], r = [], i = 0, o = 0, a = -1, l, c; for (let p = 0; p < n.length; p++) {
    let h = n[p];
...
var cg = { ";": Uh, ",": qh, ":": Hh };
function ug(n) { let e = n, t = null; for (let s = 0; s < e.length; s++) {
    let r = e[s];
    if (r === "\\")
        s++;
    else if (t !== null)
...
function hg(n, e, t = "") { let s = cn; Es.lastIndex = 0; let r = Es.test(e); if (n.length === 0)
...
function fg(n, e) { let t = n.length; for (let s = 1; s < e; s++)
    for (let r = 0; r < t; r++)
        n[r + s * t] = n[r].slice(0); }
...
var J = class extends G {
    ...
    isConstant() { return !1; }
    transformInternalExpressions(e, t) { this.body !== null ? this.body = T(this.body, e, t | z.InChildOperation) : this.fn !== null && (this.fn = T(this.fn, e, t)); for (let s = 0; s < this.args.length; s++)
    ...
    isEquivalent() { return !1; }
    isConstant() { return !1; }
    transformInternalExpressions(e, t) { this.receiver = T(this.receiver, e, t); for (let s = 0; s < this.args.length; s++)
        this.args[s] = T(this.args[s], e, t); }
...
function ha(n, e, t) { for (let s = 0; s < n.expressions.length; s++)
...
function T(n, e, t) { if (n instanceof J)
...
function yo(n, e, t) { if (n instanceof tt)
    n.expr = T(n.expr, e, t);
...
var S = (() => { class n {
    ...
    *reversed() { let t = this.tail.prev; for (; t !== this.head;) {
    ...
    static replaceWithMany(t, s) { if (s.length === 0) {
        ...
        n.assertIsUnowned(p), p.debugListId = r, a.next = p, p.prev = a, p.next = null, a = p; let l = s[0], c = a; i !== null && (i.next = l, l.prev = i), o !== null && (o.prev = c, c.next = o); }
...
function fa(n, e, t, s, r) { if (t === null && n !== Vn.Attr)
    throw new Error("AssertionError: i18nBlock must be provided for non-attribute contexts."); return E({ kind: u.I18nContext, contextKind: n, xref: e, i18nBlock: t, message: s, sourceSpan: r, params: new Map, postprocessingParams: new Map }, F); }
...
function Wg(n) { for (let e of n.units)
    for (let t of e.ops())
        fe(t, jg, z.None); }
function jg(n) { if (n instanceof Me && n.fn instanceof Ce && n.fn.name === "$any") {
    if (n.args.length !== 1)
        throw new Error("The $any builtin function expects exactly one argument.");
    return n.args[0];
} return n; }
function zg(n) { let e = new Map; for (let t of n.units)
...
function Gg(n, e) { if (e.next?.kind !== u.I18nExpression)
...
function Xg(n) { for (let e of n.units) {
...
function Yg(n) { if (!(!n.enableDebugLocations || n.relativeTemplatePath === null))
...
function Oc(n) { let e = new Map; for (let t of n.create)
    Js(t) && (e.set(t.xref, t), t.kind === u.RepeaterCreate && t.emptyView !== null && e.set(t.emptyView, t)); return e; }
...
function ms(n, e) { let t = n.get(e); if (t === void 0)
    throw new Error("All attributes should have an element-like target."); return t; }
function Zg(n, e, t) { if (e.expression instanceof ce)
    return; let s = e.isTextAttribute || e.expression.isConstant(); if (n.job.compatibility === nt.TemplateDefinitionBuilder && (s &&= e.isTextAttribute), s) {
...
function Jg(n, e) { let t = n.get(e); if (t === void 0)
    throw new Error("All attributes should have an element-like target."); return t; }
function Kg(n) { let e = new Map; for (let t of n.units)
    for (let s of t.create)
        kn(s) && e.set(s.xref, s); for (let t of n.units)
    for (let s of t.ops())
        if (s.kind === u.Binding)
...
function Tu(n) { let e = null; for (let t of n) {
...
function iv(n) { let e = new Map([["svg", be.SVG], ["math", be.Math]]); for (let [t, s] of e.entries())
    if (s === n)
...
function bu(n, e, t) { let s = e.get(t); if (s !== void 0) {
    let r = yl(s);
...
var ls = Object.freeze([]), xl = class {
    ...
    add(e, t, s, r, i) { if (!(this.compatibility === nt.TemplateDefinitionBuilder && (e === I.Attribute || e === I.ClassName || e === I.StyleProperty)) && this.isKnown(e, t))
    ...
    } let a = this.arrayFor(e); if (a.push(...lv(r, t)), e === I.Attribute || e === I.StyleProperty) {
        if (s === null)
...
function yl({ attributes: n, bindings: e, classes: t, i18n: s, projectAs: r, styles: i, template: o }) { let a = [...n]; if (r !== null) {
    let l = Qo(r)[0];
    a.push(d(5), Ws(l));
...
function uv(n) { let e = new Map; for (let r of n.units)
    for (let i of r.ops())
        switch (i.kind) {
            case u.Binding:
            case u.Property:
...
function pv(n) { let e = new Map; for (let t of n.units)
...
function fv(n) { let e = new Map; function t(r) { if (e.has(r.xref))
    return e.get(r.xref); let i = new dv; for (let o of r.create)
...
function wv(n) { for (let e of n.units)
    for (let t of e.ops())
...
function Sv(n, e, t) { return T(n, s => { if (s instanceof Ut && e.has(s.xref)) {
    let r = new bn(s.xref);
    return t.job.compatibility === nt.TemplateDefinitionBuilder ? new Ut(r, r.xref) : r;
} return s; }, z.None), n; }
function cs(n, e, t) { let s; if (We(n)) {
    let r = t.job.allocateXrefId();
    s = [new Ut(n, r), new bn(r)];
}
else
    s = [n, n.clone()], Sv(s[1], Ev(s[0]), t); return new Hs(s[0], e(s[1])); }
...
function Cv(n) { if (sf(n) && n.receiver instanceof Hs) {
    let e = n.receiver;
    for (; e.expr instanceof Hs;)
        e = e.expr;
    return e;
} return null; }
function Av(n, e) { if (!sf(n))
    return n; let t = Cv(n); if (t) {
    if (n instanceof Me)
...
function Lv(n) { let e = new Map, t = new Map, s = new Map; for (let i of n.units)
...
function Mv(n) { if (n.strings.length !== n.expressionPlaceholders.length + 1)
    throw Error(`AssertionError: Invalid ICU placeholder with ${n.strings.length} strings and ${n.expressionPlaceholders.length} expressions`); let e = n.expressionPlaceholders.map(Ss); return n.strings.flatMap((t, s) => [t, e[s] || ""]).join(""); }
function Iu(n) { let e = new Map; for (let [t, s] of n) {
    let r = Rv(s);
    r !== null && e.set(t, d(r));
} return e; }
function Rv(n) { if (n.length === 0)
    return null; let e = n.map(t => Ss(t)); return e.length === 1 ? e[0] : `${Nv}${e.join(Pv)}${Dv}`; }
function Ss(n) { if (n.flags & ee.ElementTag && n.flags & ee.TemplateTag) {
    ...
    return `${Ss(ue(E({}, n), { flags: n.flags & ~ee.CloseTag }))}${Ss(ue(E({}, n), { flags: n.flags & ~ee.OpenTag }))}`; if (n.flags === ee.None)
    return `${n.value}`; let e = "", t = ""; n.flags & ee.ElementTag ? e = Tv : n.flags & ee.TemplateTag && (e = bv), e !== "" && (t = n.flags & ee.CloseTag ? kv : ""); let s = n.subTemplateIndex === null ? "" : `${Iv}${n.subTemplateIndex}`; return `${ku}${t}${e}${n.value}${s}${ku}`; }
function Fv(n) { for (let e of n.units) {
    let t = new Map;
...
function $v(n) { for (let e of n.units)
    for (let t of e.update) {
...
function Ov(n) { let e = n.compatibility === nt.TemplateDefinitionBuilder, t = [], s = 0; for (let r of n.units)
    for (let i of r.create)
...
function or(n, e) { let t = qv(n, e); for (let s of n.create)
    switch (s.kind) {
        case u.ConditionalCreate:
...
function qv(n, e) { let t = { view: n.xref, viewContextVariable: { kind: Se.Context, name: null, view: n.xref }, contextVariables: new Map, aliases: n.aliases, references: [], letDeclarations: [], parent: e }; for (let s of n.contextVariables.keys())
...
function Di(n, e, t) { let s = []; e.view !== n.xref && s.push(hn(n.job.allocateXrefId(), e.viewContextVariable, new vo, at.None)); let r = n.job.views.get(e.view); for (let [i, o] of r.contextVariables) {
    ...
    s.push(hn(n.job.allocateXrefId(), i, i.expression.clone(), at.AlwaysInline)); for (let i of e.references)
    s.push(hn(n.job.allocateXrefId(), i.variable, new go(i.targetId, i.targetSlot, i.offset), at.None)); if (e.view !== n.xref || t)
    for (let i of e.letDeclarations)
        s.push(hn(n.job.allocateXrefId(), i.variable, new Zr(i.targetId, i.targetSlot), at.None)); return e.parent !== null && s.push(...Di(n, e.parent, !1)), s; }
...
function Wv(n) { for (let e of n.root.update)
...
`);
    ...
    tokenize() { for (; this._cursor.peek() !== Ae;) {
    ...
    _consumeBlockStart(e) { this._beginToken(24, e); let t = this._endToken([this._getBlockName()]); if (this._cursor.peek() === yt)
    ...
    _consumeBlockParameters() { for (this._attemptCharCodeUntilFn(Fu); this._cursor.peek() !== Pe && this._cursor.peek() !== Ae;) {
        this._beginToken(27);
    ...
    _consumeLetDeclaration(e) { if (this._beginToken(29, e), jr(this._cursor.peek()))
        this._attemptCharCodeUntilFn(re);
    else {
    ...
    _consumeLetDeclarationValue() { let e = this._cursor.clone(); for (this._beginToken(30, e); this._cursor.peek() !== Ae;) {
    ...
    _tokenizeExpansionForm() { if (this.isExpansionFormStart())
        return this._consumeExpansionFormStart(), !0; if (ew(this._cursor.peek()) && this._isInExpansionForm())
    ...
    handleError(e) { if (e instanceof ri && (e = this._createError(e.msg, this._cursor.getSpan(e.cursor))), e instanceof D)
        this.errors.push(e);
    else
        throw e; }
    ...
    _consumeEntity(e) { this._beginToken(9); let t = this._cursor.clone(); if (this._cursor.advance(), this._attemptCharCode(_h)) {
    ...
    _consumePrefixAndName(e) { let t = this._cursor.clone(), s = ""; for (; this._cursor.peek() !== jt && !Zv(this._cursor.peek());)
        this._cursor.advance(); let r; this._cursor.peek() === jt ? (s = this._cursor.getChars(t), this._cursor.advance(), r = this._cursor.clone()) : r = t, this._requireCharCodeUntilFn(e, s === "" ? 0 : 1); let i = this._cursor.getChars(r); return [s, i]; }
    _consumeTagOpen(e) { let t, s, r, i; try {
        ...
        throw a;
    } let o = this._getTagDefinition(t).getContentType(s); o === Lt.RAW_TEXT ? this._consumeRawTextWithTagClose(i, r, !1) : o === Lt.ESCAPABLE_RAW_TEXT && this._consumeRawTextWithTagClose(i, r, !0); }
    ...
    _consumeComponentName() { let e = this._cursor.clone(); for (; $u(this._cursor.peek());)
        this._cursor.advance(); let t = this._cursor.getChars(e), s = "", r = ""; return this._cursor.peek() === jt && (this._cursor.advance(), [s, r] = this._consumePrefixAndName(ps)), [t, s, r]; }
    ...
    _consumeAttributeName() { let e = this._cursor.peek(); if (e === Wr || e === Hr)
        throw this._createError(us(e), this._cursor.getSpan()); this._beginToken(14); let t; if (this._openDirectiveCount > 0) {
        let r = 0;
        ...
        t = ps; let s = this._consumePrefixAndName(t); this._endToken(s); }
    _consumeAttributeValue() { if (this._cursor.peek() === Wr || this._cursor.peek() === Hr) {
    ...
    } this._beginToken(3, e), this._attemptCharCodeUntilFn(re); let t = this._consumePrefixAndName(ps); this._attemptCharCodeUntilFn(re), this._requireCharCode(rt), this._endToken(t); }
    _consumeExpansionFormStart() { this._beginToken(19), this._requireCharCode(St), this._endToken([]), this._expansionCaseStack.push(19), this._beginToken(7); let e = this._readUntil(ut), t = this._processCarriageReturns(e); if (this._i18nNormalizeLineEndingsInICUs)
        this._endToken([t]);
    else {
        let r = this._endToken([e]);
        t !== e && this.nonNormalizedIcuExpressions.push(r);
    } this._requireCharCode(ut), this._attemptCharCodeUntilFn(re), this._beginToken(7); let s = this._readUntil(ut); this._endToken([s]), this._requireCharCode(ut), this._attemptCharCodeUntilFn(re); }
    ...
    _consumeInterpolation(e, t, s) { let r = []; this._beginToken(e, t), r.push(this._interpolationConfig.start); let i = this._cursor.clone(), o = null, a = !1; for (; this._cursor.peek() !== Ae && (s === null || !s());) {
    ...
    _consumeDirective(e, t) { for (this._requireCharCode(er), this._cursor.advance(); $u(this._cursor.peek());)
        this._cursor.advance(); this._beginToken(38, e); let s = this._cursor.getChars(t); if (this._endToken([s]), this._attemptCharCodeUntilFn(re), this._cursor.peek() === yt) {
    ...
    isExpansionFormStart() { if (this._cursor.peek() !== St)
...
function sw(n) { let e = [], t; for (let s = 0; s < n.length; s++) {
    let r = n[s];
...
var No = class n {
    ...
    advanceState(e) { if (e.offset >= this.end)
        throw this.state = e, new ri('Unexpected character "EOF"', this); let t = this.charAt(e.offset); t === Wn ? (e.line++, e.column = 0) : Mc(t) || e.column++, e.offset++, this.updatePeek(e); }
    ...
    processEscapeSequence() { let e = () => this.internalState.peek; if (e() === _s)
    ...
    decodeHexDigits(e, t) { let s = this.input.slice(e.internalState.offset, e.internalState.offset + t), r = parseInt(s, 16); if (isNaN(r))
        throw e.state = e.internalState, new ri("Invalid hexadecimal escape sequence", e); return r; }
    ...
    _consumeExpansion(e) { let t = this._advance(), s = this._advance(), r = []; for (; this._peek.type === 20;) {
        ...
        return;
    } let i = new B(e.sourceSpan.start, this._peek.sourceSpan.end, e.sourceSpan.fullStart); this._addToParent(new gn(t.parts[0], s.parts[0], r, i, t.sourceSpan)), this._advance(); }
    _parseExpansionCase() { let e = this._advance(); if (this._peek.type !== 21)
        return this.errors.push(he.create(null, this._peek.sourceSpan, "Invalid ICU message. Missing '{'.")), null; let t = this._advance(), s = this._collectExpansionExpTokens(t); if (!s)
        ...
        return this.errors = this.errors.concat(i.errors), null; let o = new B(e.sourceSpan.start, r.sourceSpan.end, e.sourceSpan.fullStart), a = new B(t.sourceSpan.start, r.sourceSpan.end, t.sourceSpan.fullStart); return new ni(e.parts[0], i.rootNodes, o, e.sourceSpan, a); }
...
`) {
    ...
    _consumeElementStartTag(e) { let t = [], s = []; this._consumeAttributesAndDirectives(t, s); let r = this._getElementFullName(e, this._getClosestElementLikeParent()), i = !1; if (this._peek.type === 2) {
        this._advance(), i = !0;
        ...
        this._peek.type === 1 && (this._advance(), i = !1); let o = this._peek.sourceSpan.fullStart, a = new B(e.sourceSpan.start, o, e.sourceSpan.fullStart), l = new B(e.sourceSpan.start, o, e.sourceSpan.fullStart), c = new De(r, t, s, [], i, a, l, void 0), p = this._getContainer(), h = p !== null && !!this._getTagDefinition(p)?.isClosedByChild(c.name); this._pushContainer(c, h), i ? this._popContainer(r, De, a) : e.type === 4 && (this._popContainer(r, De, null), this.errors.push(he.create(r, a, `Opening tag "${r}" not terminated.`))); }
    ...
    _consumeComponentEndTag(e) { let t = this._getComponentFullName(e, this._getClosestElementLikeParent()); if (!this._popContainer(t, ye, e.sourceSpan)) {
    ...
    _consumeAttr(e) { let t = Ni(e.parts[0], e.parts[1]), s = e.sourceSpan.end; this._peek.type === 15 && this._advance(); let r = "", i = [], o, a; if (this._peek.type === 16)
        ...
        } this._peek.type === 15 && (s = this._advance().sourceSpan.end); let c = o && a && new B(o.start, a, o.fullStart); return new vn(t, r, new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), e.sourceSpan, c, i.length > 0 ? i : void 0, void 0); }
    ...
    } let i = new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), o = new B(i.start, r === null ? e.sourceSpan.end : r.end, i.fullStart); return new bo(e.parts[0], t, o, i, r); }
    _consumeBlockOpen(e) { let t = []; for (; this._peek.type === 27;) {
        let a = this._advance();
        t.push(new ko(a.parts[0], a.sourceSpan));
    } this._peek.type === 25 && this._advance(); let s = this._peek.sourceSpan.fullStart, r = new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), i = new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), o = new it(e.parts[0], t, [], r, e.sourceSpan, i); this._pushContainer(o, !1); }
    ...
    _consumeIncompleteBlock(e) { let t = []; for (; this._peek.type === 27;) {
        let a = this._advance();
        t.push(new ko(a.parts[0], a.sourceSpan));
    } let s = this._peek.sourceSpan.fullStart, r = new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), i = new B(e.sourceSpan.start, s, e.sourceSpan.fullStart), o = new it(e.parts[0], t, [], r, e.sourceSpan, i); this._pushContainer(o, !1), this._popContainer(null, it, null), this.errors.push(he.create(e.parts[0], r, `Incomplete block "${e.parts[0]}". If you meant to write the @ character, you should use the "&#64;" HTML entity instead.`)); }
    _consumeLet(e) { let t = e.parts[0], s, r; if (this._peek.type !== 30) {
        ...
        r = this._advance(); let i = r.sourceSpan.fullStart, o = new B(e.sourceSpan.start, i, e.sourceSpan.fullStart), a = e.sourceSpan.toString().lastIndexOf(t), l = e.sourceSpan.start.moveBy(a), c = new B(l, e.sourceSpan.end), p = new Io(t, s.parts[0], o, c, s.sourceSpan); this._addToParent(p); }
...
var ii = class {
    ...
    visitElement(e, t) { if (Uu.has(e.name) || Hu(e.attrs)) {
    ...
    } let s = new De(e.name, e.attrs, e.directives, gt(this, e.children), e.isSelfClosing, e.sourceSpan, e.startSourceSpan, e.endSourceSpan, e.i18n); return this.originalNodeMap?.set(s, e), s; }
    visitAttribute(e, t) { return e.name !== lf ? e : null; }
    visitText(e, t) { let s = e.value.match(rw), r = t && (t.prev instanceof gn || t.next instanceof gn); if (this.icuExpansionDepth > 0 && this.preserveSignificantWhitespace)
        return e; if (s || r) {
    ...
    visitComponent(e, t) { if (e.tagName && Uu.has(e.tagName) || Hu(e.attrs)) {
        ...
        return this.originalNodeMap?.set(r, e), r;
    } let s = new ye(e.componentName, e.tagName, e.fullName, e.attrs, e.directives, gt(this, e.children), e.isSelfClosing, e.sourceSpan, e.startSourceSpan, e.endSourceSpan, e.i18n); return this.originalNodeMap?.set(s, e), s; }
...
var xi = new Nt(-1, -1, $.Character, 0, ""), Il = class {
    ...
    scanToken() { let e = this.input, t = this.length, s = this.peek, r = this.index; for (; s <= Ah;)
        if (++r >= t) {
            s = Ae;
            break;
        }
        ...
        return null; if (Wu(s))
        return this.scanIdentifier(); if (wt(s))
    ...
    scanIdentifier() { let e = this.index; for (this.advance(); ju(this.peek);)
        this.advance(); let t = this.input.substring(e, this.index); return uw.indexOf(t) > -1 ? fw(e, this.index, t) : pw(e, this.index, t); }
    scanPrivateIdentifier() { let e = this.index; if (this.advance(), !Wu(this.peek))
        return this.error("Invalid character [#]", -1); for (; ju(this.peek);)
        this.advance(); let t = this.input.substring(e, this.index); return hw(e, this.index, t); }
    scanNumber(e) { let t = this.index === e, s = !1; for (this.advance();;) {
        if (!wt(this.peek))
            if (this.peek === Pn) {
                ...
                break;
        this.advance();
    } let r = this.input.substring(e, this.index); s && (r = r.replace(/_/g, "")); let i = t ? Ew(r) : parseFloat(r); return dw(e, this.index, i); }
    scanString() { let e = this.index, t = this.peek; this.advance(); let s = "", r = this.index, i = this.input; for (; this.peek != t;)
        if (this.peek == _s) {
            ...
            this.advance();
        } let o = i.substring(r, this.index); return this.advance(), new dr(e, this.index, s + o, jn.Plain); }
    ...
    scanTemplateLiteralPart(e) { let t = "", s = this.index; for (; this.peek !== ul;)
        if (this.peek === _s) {
            ...
            this.advance();
        } let r = this.input.substring(s, this.index); return this.advance(), new dr(e, this.index, t + r, jn.TemplateLiteralEnd); }
    error(e, t) { let s = this.index + t; return mw(s, this.index, `Lexer Error: ${e} at column ${s} in expression [${this.input}]`); }
    scanStringBackslash(e, t) { e += this.input.substring(t, this.index); let s; if (this.advance(), this.peek === Dh) {
        ...
        s = ww(this.peek), this.advance(); return e += String.fromCharCode(s), e; }
...
function Ew(n) { let e = parseInt(n); if (isNaN(e))
...
var Do = class {
    ...
    parseInterpolation(e, t, s, r, i = ot) { let o = [], { strings: a, expressions: l, offsets: c } = this.splitInterpolation(e, t, o, r, i); if (l.length === 0)
        return null; let p = []; for (let h = 0; h < l.length; ++h) {
    ...
    splitInterpolation(e, t, s, r, i = ot) { let o = [], a = [], l = [], c = r ? Sw(r) : null, p = 0, h = !1, m = !1, { start: v, end: w } = i; for (; p < e.length;)
        if (h) {
        ...
        if (m) {
            ...
            o.push({ text: e.substring(p), start: p, end: e.length }); return new Nl(o, a, l); }
    ...
    _checkNoInterpolation(e, t, s, { start: r, end: i }) { let o = -1, a = -1; for (let l of this._forEachUnquotedChar(t, 0))
        if (o === -1)
            t.startsWith(r) && (o = l);
        else if (a = this._getInterpolationEndIndex(t, i, l), a > -1)
            break; o > -1 && a > -1 && e.push(mr(`Got interpolation (${r}${i}) where expression was expected`, t, `at column ${o} in`, s)); }
    ...
    *_forEachUnquotedChar(e, t) { let s = null, r = 0; for (let i = t; i < e.length; i++) {
    ...
    parseEquality() { let e = this.inputIndex, t = this.parseRelational(); for (; this.next.type == $.Operator;) {
    ...
    parseRelational() { let e = this.inputIndex, t = this.parseAdditive(); for (; this.next.type == $.Operator || this.next.isKeywordIn;) {
        let s = this.next.strValue;
        switch (s) {
            case "<":
            case ">":
    ...
    parseAdditive() { let e = this.inputIndex, t = this.parseMultiplicative(); for (; this.next.type == $.Operator;) {
        let s = this.next.strValue;
        switch (s) {
            case "+":
            case "-":
    ...
    parseMultiplicative() { let e = this.inputIndex, t = this.parseExponentiation(); for (; this.next.type == $.Operator;) {
        let s = this.next.strValue;
        switch (s) {
            case "*":
            case "%":
    ...
    parseExponentiation() { let e = this.inputIndex, t = this.parsePrefix(); for (; this.next.type == $.Operator && this.next.strValue === "**";) {
    ...
    parsePrefix() { if (this.next.type == $.Operator) {
    ...
    parseCallChain() { let e = this.inputIndex, t = this.parsePrimary(); for (;;)
        if (this.consumeOptionalCharacter(ir))
            ...
            return t; }
    parsePrimary() { let e = this.inputIndex; if (this.consumeOptionalCharacter(yt)) {
    ...
    parseExpressionList(e) { let t = []; do
        if (!this.next.isCharacter(e))
            ...
            break;
    while (this.consumeOptionalCharacter(ut)); return t; }
    parseLiteralMap() { let e = [], t = [], s = this.inputIndex; if (this.expectCharacter(St), !this.consumeOptionalCharacter(Fe)) {
    ...
    parseAccessMember(e, t, s) { let r = this.inputIndex, i = this.withContext(ar.Writable, () => { let a = this.expectIdentifierOrKeyword() ?? ""; return a.length === 0 && this.error("Expected identifier for property access", e.span.end), a; }), o = this.sourceSpan(r); if (s)
        return this.isAssignmentOperator(this.next) ? (this.advance(), this.error("The '?.' operator cannot be used in the assignment"), new Re(this.span(t), this.sourceSpan(t))) : new Hi(this.span(t), this.sourceSpan(t), o, e, i); if (this.isAssignmentOperator(this.next)) {
    ...
    while (this.consumeOptionalCharacter(ut)); return e; }
    ...
    parseKeyedReadOrWrite(e, t, s) { return this.withContext(ar.Writable, () => { this.rbracketsExpected++; let r = this.parsePipe(); if (r instanceof Re && this.error("Key access cannot be empty"), this.rbracketsExpected--, this.expectCharacter(ws), this.isAssignmentOperator(this.next)) {
        ...
        return s ? new Wi(this.span(t), this.sourceSpan(t), e, r) : new Ls(this.span(t), this.sourceSpan(t), e, r); return new Re(this.span(t), this.sourceSpan(t)); }); }
    ...
    getDirectiveBoundTarget() { if (this.next === xi || this.peekKeywordAs() || this.peekKeywordLet())
        return null; let e = this.parsePipe(), { start: t, end: s } = e.span, r = this.input.substring(t, s); return new Ge(e, r, un(this.parseSourceSpan), this.absoluteOffset + t, this.errors); }
    parseAsBinding(e) { if (!this.peekKeywordAs())
        return null; this.advance(); let t = this.expectTemplateBindingKey(); this.consumeStatementTerminator(); let s = new Qe(e.span.start, this.currentAbsoluteOffset); return new Br(s, t, e); }
    parseLetBinding() { if (!this.peekKeywordLet())
        return null; let e = this.currentAbsoluteOffset; this.advance(); let t = this.expectTemplateBindingKey(), s = null; this.consumeOptionalOperator("=") && (s = this.expectTemplateBindingKey()), this.consumeStatementTerminator(); let r = new Qe(e, this.currentAbsoluteOffset); return new Br(r, t, s); }
    ...
    parseTemplateLiteral() { let e = [], t = [], s = this.inputIndex; for (; this.next !== xi;) {
        let r = this.next;
    ...
    skip() { let e = this.next; for (; this.index < this.tokens.length && !e.isCharacter(Xe) && !e.isOperator("|") && (this.rparensExpected <= 0 || !e.isCharacter(Pe)) && (this.rbracesExpected <= 0 || !e.isCharacter(Fe)) && (this.rbracketsExpected <= 0 || !e.isCharacter(ws)) && (!(this.context & ar.Writable) || !this.isAssignmentOperator(e));)
...
function Sw(n) { let e = new Map, t = 0, s = 0, r = 0; for (; r < n.length;) {
    let i = n[r];
    if (i.type === 9) {
        let [o, a] = i.parts;
...
function yw(n, e) { if (n.length !== e.length)
    throw new Error("Array lengths must match"); return n.map((t, s) => [t, e[s]]); }
function Cw(n, e) { let t = []; for (let s = 0; s < Math.max(n.length, e.length); s++)
...
function yi(n, e) { for (let t of e)
    Pi[t.toLowerCase()] = n; }
...
var Bl = class {
    ...
    constructor() { super(), Nw.forEach(e => { let t = new Map, s = new Set, [r, i] = e.split("|"), o = i.split(","), [a, l] = r.split("^"); a.split(",").forEach(p => { this._schema.set(p.toLowerCase(), t), this._eventSchema.set(p.toLowerCase(), s); }); let c = l && this._schema.get(l.toLowerCase()); if (c) {
        for (let [p, h] of c)
            t.set(p, h);
    ...
    hasProperty(e, t, s) { if (s.some(i => i.name === La.name))
...
If '${e}' is a directive input, make sure the directive is imported by the current module.` } : { error: !1 };
    ...
    normalizeAnimationStyleValue(e, t, s) { let r = "", i = s.toString().trim(), o = null; if (Pw(e) && s !== 0 && s !== "0")
        if (typeof s == "number")
            r = "px";
        else {
            let a = s.match(/^[+-]?[\d\.]+([a-z]*)$/);
...
var Xu = { A: "LINK", B: "BOLD_TEXT", BR: "LINE_BREAK", EM: "EMPHASISED_TEXT", H1: "HEADING_LEVEL1", H2: "HEADING_LEVEL2", H3: "HEADING_LEVEL3", H4: "HEADING_LEVEL4", H5: "HEADING_LEVEL5", H6: "HEADING_LEVEL6", HR: "HORIZONTAL_RULE", I: "ITALIC_TEXT", LI: "LIST_ITEM", LINK: "MEDIA_LINK", OL: "ORDERED_LIST", P: "PARAGRAPH", Q: "QUOTATION", S: "STRIKETHROUGH_TEXT", SMALL: "SMALL_TEXT", SUB: "SUBSTRIPT", SUP: "SUPERSCRIPT", TBODY: "TABLE_BODY", TD: "TABLE_CELL", TFOOT: "TABLE_FOOTER", TH: "TABLE_HEADER_CELL", THEAD: "TABLE_HEADER", TR: "TABLE_ROW", TT: "MONOSPACED_TEXT", U: "UNDERLINED_TEXT", UL: "UNORDERED_LIST" }, Rl = class {
    ...
    getStartTagPlaceholderName(e, t, s) { let r = this._hashTag(e, t, s); if (this._signatureToName[r])
        return this._signatureToName[r]; let i = e.toUpperCase(), o = Xu[i] || `TAG_${i}`, a = this._generateUniqueName(s ? o : `START_${o}`); return this._signatureToName[r] = a, a; }
    getCloseTagPlaceholderName(e) { let t = this._hashClosingTag(e); if (this._signatureToName[t])
        return this._signatureToName[t]; let s = e.toUpperCase(), r = Xu[s] || `TAG_${s}`, i = this._generateUniqueName(`CLOSE_${r}`); return this._signatureToName[t] = i, i; }
...
var Fl = class {
    ...
    visitExpansion(e, t) { t.icuDepth++; let s = {}, r = new An(e.switchValue, e.type, s, e.sourceSpan); if (e.cases.forEach(a => { s[a.value] = new Ke(a.expression.map(l => l.visit(this, t)), a.expSourceSpan); }), t.icuDepth--, t.isIcu || t.icuDepth > 0) {
    ...
    } let i = t.placeholderRegistry.getPlaceholderName("ICU", e.sourceSpan.toString()); t.placeholderToMessage[i] = this.toI18nMessage([e], "", "", "", void 0); let o = new _n(r, i, e.sourceSpan); return t.visitNodeFn(e, o); }
    visitExpansionCase(e, t) { throw new Error("Unreachable code"); }
    visitBlock(e, t) { let s = L(this, e.children, t); if (this._containerBlocks.has(e.name))
        return new Ke(s, e.sourceSpan); let r = e.parameters.map(l => l.expression), i = t.placeholderRegistry.getStartBlockPlaceholderName(e.name, r), o = t.placeholderRegistry.getCloseBlockPlaceholderName(e.name); t.placeholderToContent[i] = { text: e.startSourceSpan.toString(), sourceSpan: e.startSourceSpan }, t.placeholderToContent[o] = { text: e.endSourceSpan ? e.endSourceSpan.toString() : "}", sourceSpan: e.endSourceSpan ?? e.sourceSpan }; let a = new Vt(e.name, r, i, o, s, e.sourceSpan, e.startSourceSpan, e.endSourceSpan); return t.visitNodeFn(e, a); }
...
function Mw(n, e) { if (e instanceof ke && (Rw(e), e = e.nodes[0]), e instanceof Ke) {
    Fw(e.children, n);
    for (let t = 0; t < n.length; t++)
        n[t].sourceSpan = e.children[t].sourceSpan;
} }
function Rw(n) { let e = n.nodes; if (e.length !== 1 || !(e[0] instanceof Ke))
...
var Vw = n => (e, t) => { let s = n.get(e) ?? e; return s instanceof tn && (t instanceof _n && s.i18n instanceof ke && (t.previousMessage = s.i18n), s.i18n = t), t; }, Po = class {
    ...
    visitExpansion(e, t) { let s, r = e.i18n; if (this.hasI18nMeta = !0, r instanceof _n) {
        let i = r.name;
...
function Hw(n = "") { let e, t, s; if (n = n.trim(), n) {
...
var Ol = class {
    ...
    visitText(e) { if (this.pieces[this.pieces.length - 1] instanceof dn)
...
function Jw(n) { let e = [], t = []; n[0] instanceof Un && e.push(ma(n[0].sourceSpan.start)); for (let s = 0; s < n.length; s++) {
    let r = n[s];
...
function sE(n) { let e = n.relativeContextFilePath.replace(/[^A-Za-z0-9]/g, "_").toUpperCase() + "_", t = new Map, s = new Map, r = new Map, i = new Map; for (let l of n.units)
    for (let c of l.ops())
    ...
    for (let c of l.create)
        if (kn(c)) {
            ...
            let v = h.flatMap(w => { let C = o.get(w.context); if (C === void 0)
...
function mf(n, e, t, s) { let r = [], i = new Map; for (let c of s.subMessages) {
    let p = t.get(c), { mainVar: h, statements: m } = mf(n, e, t, p);
    r.push(...m);
    let v = i.get(p.messagePlaceholder) ?? [];
    v.push(h), i.set(p.messagePlaceholder, v);
} rE(s, i), s.params = new Map([...s.params.entries()].sort()); let o = N(n.pool.uniqueName(Kw)), a = aE(n.pool, s.message.id, e, n.i18nUseExternalIds), l; if (s.needsPostprocessing || s.postprocessingParams.size > 0) {
...
function rE(n, e) { for (let [t, s] of e)
...
function aE(n, e, t, s) { let r, i = t; if (s) {
...
function lE(n) { for (let e of n.units) {
...
function uE(n) { let e = []; for (let t of n)
...
function hE(n) { let e = [], t = 0, s = 0, r = 0, i = 0, o = 0, a = null; for (; t < n.length;)
...
function fE(n) { let e = new Map; for (let t of n.units)
    for (let s of t.create)
        kn(s) && e.set(s.xref, s); for (let t of n.units)
    for (let s of t.create)
...
function lr(n, e, t, s) { n.fnName === null && (n.fnName = n.job.pool.uniqueName(fr(`${e}_${n.job.fnSuffix}`), !1)); let r = new Map; for (let i of n.ops())
    switch (i.kind) {
        case u.Property:
        case u.DomProperty:
    ...
    xe(i, o => { if (!(!(o instanceof Zt) || o.name !== null)) {
        if (!r.has(o.xref))
            throw new Error(`Variable ${o.xref} not yet named`);
        o.name = r.get(o.xref);
    } }); }
function mE(n, e, t) { if (e.name === null)
    switch (e.kind) {
        case Se.Context:
...
function SE(n, e) { let t = n.get(e); if (t === void 0)
    throw new Error("All attributes should have an element-like target."); return t; }
...
function bE(n) { for (let e of n.units) {
    sp(e.create, AE);
    let t = e.job.kind === A.Host ? TE : _E;
    sp(e.update, t);
} }
function sp(n, e) { let t = [], s = null; for (let r of n) {
...
function rp(n, e) { let t = Array.from(e, () => new Array); for (let s of n) {
...
function kE(n) { for (let e of n.units) {
    let t = Oc(e);
    for (let s of e.ops())
        switch (s.kind) {
            case u.Binding:
...
function NE(n, e) { let t = n.get(e); if (t === void 0)
    throw new Error("All attributes should have an slottable target."); return t; }
function DE(n) { for (let e of n.units)
    PE(e); }
function PE(n) { for (let e of n.update)
    xe(e, (t, s) => { if (qt(t) && t.kind === k.PipeBinding) {
...
function LE(n, e, t) { for (let s = n.create.head.next; s.kind !== u.ListEnd; s = s.next) {
    if (!Js(s) || s.xref !== e)
        continue;
...
function ga(n, e, t, s) { if (t !== void 0) {
    if (e === null)
        throw Error("Expected template with i18n placeholder to be in an i18n block.");
    s++, RE(n, e);
} return vf(n, s); }
function RE(n, e) { if (n.create.head.next?.kind !== u.I18nStart) {
...
function FE(n) { for (let e of n.units)
    for (let t of e.ops())
        xe(t, s => { if (!(s instanceof rs) || s.body === null)
            return; let r = new Vl(s.args.length); s.fn = n.pool.getSharedConstant(r, s.body), s.body = null; }); }
var Vl = class extends xs {
    ...
    toSharedConstantDeclaration(e, t) { let s = []; for (let i = 0; i < this.numArgs; i++)
        s.push(new Z("a" + i)); let r = T(t, i => i instanceof Vs ? N("a" + i.index) : i, z.None); return new le(e, new En(s, r), void 0, ae.Final); }
};
function $E(n) { for (let e of n.units)
    for (let t of e.update)
        fe(t, (s, r) => r & z.InChildOperation ? s : s instanceof ft ? OE(s) : s instanceof bt ? VE(s) : s, z.None); }
function OE(n) { let e = [], t = []; for (let s of n.entries)
    if (s.isConstant())
        e.push(s);
    else {
        let r = t.length;
...
function VE(n) { let e = [], t = []; for (let s of n.entries)
...
function wf(n, e, t, s, r, i, o, a, l) { let c = [d(e), t, d(s), d(r), d(i), d(o)]; for (a !== null && (c.push(d(a)), c.push(y(f.templateRefExtractor))); c[c.length - 1].isEquivalent(Sn);)
    c.pop(); return O(n, c, l); }
...
function cS(n, e, t, s, r, i, o, a, l, c, p) { let h = [d(n), d(e), t ?? d(null), d(s), d(r), d(i), o ?? d(null), a ?? d(null), l ? y(f.deferEnableTimerScheduling) : d(null), d(p)], m; for (; (m = h[h.length - 1]) !== null && m instanceof Ee && m.value === null;)
    h.pop(); return O(f.defer, h, c); }
...
function pS(n, e, t, s) { let r = uS.get(n)?.[t]; if (r === void 0)
    throw new Error(`Unable to determine instruction for trigger ${n}`); return O(r, e.map(i => d(i)), s); }
...
function mS(n, e, t, s, r, i, o, a) { let l = [d(n), e, d(t), d(s), d(r), d(i)]; for (o !== null && (l.push(d(o)), l.push(y(f.templateRefExtractor))); l[l.length - 1].isEquivalent(Sn);)
    l.pop(); return O(f.conditionalCreate, l, a); }
function gS(n, e, t, s, r, i, o, a) { let l = [d(n), e, d(t), d(s), d(r), d(i)]; for (o !== null && (l.push(d(o)), l.push(y(f.templateRefExtractor))); l[l.length - 1].isEquivalent(Sn);)
    l.pop(); return O(f.conditionalBranchCreate, l, a); }
...
function qS(n, e, t) { if (t.length < 1 || t.length > op.length)
    throw new Error("pipeBind() argument count out of bounds"); let s = op[t.length - 1]; return y(s).callFn([d(n), d(e), ...t]); }
...
function Sf(n, e) { if (n.length < 1 || e.length !== n.length - 1)
...
var QS = { constant: [f.textInterpolate, f.textInterpolate1, f.textInterpolate2, f.textInterpolate3, f.textInterpolate4, f.textInterpolate5, f.textInterpolate6, f.textInterpolate7, f.textInterpolate8], variable: f.textInterpolateV, mapping: n => { if (n % 2 === 0)
        throw new Error("Expected odd number of arguments"); return (n - 1) / 2; } }, ZS = { constant: [f.interpolate, f.interpolate1, f.interpolate2, f.interpolate3, f.interpolate4, f.interpolate5, f.interpolate6, f.interpolate7, f.interpolate8], variable: f.interpolateV, mapping: n => { if (n % 2 === 0)
        throw new Error("Expected odd number of arguments"); return (n - 1) / 2; } }, JS = { constant: [f.pureFunction0, f.pureFunction1, f.pureFunction2, f.pureFunction3, f.pureFunction4, f.pureFunction5, f.pureFunction6, f.pureFunction7, f.pureFunction8], variable: f.pureFunctionV, mapping: n => n };
function Vc(n, e, t, s, r) { let i = n.mapping(t.length), o = t.at(-1); if (s.length === 0 && t.length > 1 && o instanceof Ee && o.value === "" && t.pop(), i < n.constant.length)
...
function n2(n, e) { for (let t of e)
    switch (fe(t, xf, z.None), t.kind) {
        ...
        case u.RepeaterCreate:
            ...
            break;
        case u.SourceLocation:
            let _ = R(t.locations.map(({ targetSlot: P, offset: K, line: ne, column: de }) => { if (P.slot === null)
                throw new Error("No slot was assigned for source location"); return R([d(P.slot), d(K), d(ne), d(de)]); }));
...
function qc(n, e) { for (let t of e)
    switch (fe(t, xf, z.None), t.kind) {
        case u.Advance:
...
function cp(n, e, t, s) { qc(n, t); let r = []; for (let o of t) {
    if (o.kind !== u.Statement)
        throw new Error(`AssertionError: expected reified statements, but found op ${u[o.kind]}`);
    r.push(o.statement);
} let i = []; return s && i.push(new Z("$event")), en(i, r, void 0, void 0, e); }
function s2(n, e) { if (e.trackByFn !== null)
    return e.trackByFn; let t = [new Z("$index"), new Z("$item")], s; if (e.trackByOps === null)
...
function Bo(n, e) { let t = new Map; t.set(n.xref, N("ctx")); for (let s of e)
    switch (s.kind) {
        case u.Variable:
...
function p2(n) { let e = new Map, t = new Map; for (let s of n.units)
...
function ln(n, e, t, s, r) { let i = null, o = new Map; for (let a of e.create)
    switch (a.kind) {
        case u.I18nStart:
            if (!a.context)
...
function hp(n, e, t, s) { let { closeName: r } = n.i18nPlaceholder; if (r) {
...
function yf(n, e, t) { for (let s of t.create)
...
function h2(n) { let e = new Map, t = new Map, s = new Map; for (let o of n.units)
    for (let a of o.create)
        switch (a.kind) {
            ...
            case u.IcuPlaceholder:
                s.set(a.xref, a);
                break;
        } let r = new Map, i = o => o.usage === Qs.I18nText ? o.i18nOwner : o.context; for (let o of n.units)
...
function f2(n, e, t, s) { if (n.i18nPlaceholder !== null) {
...
function Mo(n, e, t) { let s = new Map, r = new Map; for (let i of e)
    switch (i.kind) {
        case u.Variable:
    ...
    i.kind == u.Listener || i.kind === u.TwoWayListener || fe(i, o => { if (o instanceof Ce)
        return r.has(o.name) ? new Zt(r.get(o.name)) : s.has(o.name) ? new Zt(s.get(o.name)) : new Ue(new Tn(n.job.root.xref), o.name); if (o instanceof wo && typeof o.view == "number") {
...
function fp(n) { if (Array.isArray(n)) {
    if (n.length > 1)
...
function S2(n, e) { e.handlerOps.prepend([hn(n.job.allocateXrefId(), { kind: Se.Context, name: null, view: n.xref }, new wo(n.xref), at.None)]); for (let t of e.handlerOps)
    t.kind === u.Statement && t.statement instanceof _e && (t.statement.value = new El(t.statement.value)); }
function x2(n) { let e = new Map; for (let t of n.units) {
    let s = 0;
...
function y2(n) { let e = new Set, t = new Map; for (let s of n.units)
    for (let r of s.ops())
        r.kind === u.DeclareLet && t.set(r.xref, r), xe(r, i => { i instanceof Zr && e.add(i.target); }); for (let s of n.units)
...
function A2(n) { let e = new Set; for (let t of n.units)
    for (let s of t.ops())
        xe(s, r => { if (r instanceof H)
            switch (r.operator) {
...
function Ro(n) { let e = 0, t = []; for (let s of n) {
    let r = new Map;
...
function mp(n, e) { let t = n.get(e.xref); if (t === void 0)
    ...
    for (let t of e.create)
        if (t.kind === u.RepeaterCreate)
            if (t.track instanceof et && t.track.name === "$index")
                ...
                t.track = T(t.track, r => { if (r instanceof is || r instanceof Jr)
                    throw new Error("Illegal State: Pipes are not allowed in this context"); return r instanceof Tn ? (t.usesComponentInstance = !0, new vl(r.view)) : r; }, z.None);
                let s = new S;
                s.push(It(new _e(t.track, t.track.sourceSpan))), t.trackByOps = s;
            } }
function N2(n, e) { if (!(e instanceof Me) || e.args.length === 0 || e.args.length > 2 || !(e.receiver instanceof Ue && e.receiver.receiver instanceof Tn) || e.receiver.receiver.view !== n)
    return !1; let [t, s] = e.args; return !(t instanceof et) || t.name !== "$index" ? !1 : e.args.length === 1 ? !0 : !(!(s instanceof et) || s.name !== "$item"); }
function D2(n) { for (let e of n.units)
    for (let t of e.create)
        t.kind === u.RepeaterCreate && (t.track = T(t.track, s => { if (s instanceof Ce) {
...
function P2(n) { for (let e of n.units)
    for (let t of e.create)
        t.kind === u.TwoWayListener && fe(t, s => { if (!(s instanceof Eo))
            return s; let { target: r, value: i } = s; if (r instanceof Ue || r instanceof Mt)
            return ip(r, i).or(r.set(i)); if (r instanceof Zt)
            return ip(r, i); throw new Error("Unsupported expression in two-way action binding."); }, z.InChildOperation); }
function L2(n) { for (let e of n.units) {
    let t = 0;
    for (let s of e.ops())
        pa(s) && (t += B2(s));
...
function B2(n) { let e; switch (n.kind) {
...
function Ci(n) { let e = new Map; for (let t of n)
    t.kind === u.Variable && t.flags & at.AlwaysInline && (xe(t, s => { if (qt(s) && Uc(s) !== ve.None)
        throw new Error("AssertionError: A context-sensitive variable was marked AlwaysInline"); }), e.set(t.xref, t)), fe(t, s => s instanceof Zt && e.has(s.xref) ? e.get(s.xref).initializer.clone() : s, z.None); for (let t of e.values())
    S.remove(t); }
function Ai(n, e) { let t = new Map, s = new Map, r = new Set, i = new Map; for (let c of n) {
...
function F2(n) { let e = ve.None, t = new Set; return xe(n, s => { if (qt(s))
    switch (s.kind) {
        case k.ReadVariable:
            t.add(s.xref);
            break;
...
function $2(n, e, t) { xe(n, (s, r) => { if (!qt(s) || s.kind !== k.ReadVariable)
    return; let i = e.get(s.xref); i !== void 0 && (e.set(s.xref, i + 1), r & z.InChildOperation && t.add(s.xref)); }); }
function O2(n, e) { xe(n, t => { if (!qt(t) || t.kind !== k.ReadVariable)
...
function Cf(n, e) { if (n & ve.ViewContextWrite) {
    if (e & ve.ViewContextRead)
        return !1;
}
else if (n & ve.ViewContextRead && e & ve.ViewContextWrite)
    return !1; return !0; }
function V2(n, e, t, s) { let r = !1, i = !0; return fe(t, (o, a) => { if (!qt(o) || r || !i)
    return o; if (a & z.InChildOperation && s & ve.ViewContextRead)
...
function q2(n, e) { switch (n.variable.kind) {
...
function Tf(n) { if (n.fnName === null)
    ...
    t.push(i.statement);
} let s = Fo(1, e), r = Fo(2, t); return en([new Z("rf"), new Z("ctx")], [...s, ...r], void 0, void 0, n.fnName); }
function Fo(n, e) { return e.length === 0 ? [] : [pi(new H(x.BitwiseAnd, N("rf"), d(n)), e)]; }
function j2(n) { if (n.root.fnName === null)
    ...
    return null; let s = Fo(1, e), r = Fo(2, t); return en([new Z("rf"), new Z("ctx")], [...s, ...r], void 0, void 0, n.root.fnName); }
...
function X2(n, e, t) { let s = new _o(n.componentName, t, bf, je.DomOnly); for (let r of n.properties ?? []) {
    let i = I.Property;
...
function sn(n, e) { for (let t of e)
    if (t instanceof Ft)
        J2(n, t);
...
function J2(n, e) { if (e.i18n !== void 0 && !(e.i18n instanceof ke || e.i18n instanceof Ot))
    throw Error(`Unhandled i18n metadata type for element: ${e.i18n.constructor.name}`); let t = n.job.allocateXrefId(), [s, r] = kt(e.name), i = kg(r, t, nf(s), e.i18n instanceof Ot ? e.i18n : void 0, e.startSourceSpan, e.sourceSpan); n.create.push(i), ux(n, i, e), Pf(i, e); let o = null; e.i18n instanceof ke && (o = n.job.allocateXrefId(), n.create.push(ra(o, e.i18n, void 0, e.startSourceSpan))), sn(n, e.children); let a = Ng(t, e.endSourceSpan ?? e.startSourceSpan); n.create.push(a), o !== null && S.insertBefore(ia(o, e.endSourceSpan ?? e.startSourceSpan), a); }
function K2(n, e) { if (e.i18n !== void 0 && !(e.i18n instanceof ke || e.i18n instanceof Ot))
    throw Error(`Unhandled i18n metadata type for template: ${e.i18n.constructor.name}`); let t = n.job.allocateView(n.xref), s = e.tagName, r = ""; e.tagName && ([r, s] = kt(e.tagName)); let i = e.i18n instanceof Ot ? e.i18n : void 0, o = nf(r), a = s === null ? "" : ov(s, o), l = cx(e) ? Ve.NgTemplate : Ve.Structural, c = Xh(t.xref, l, s, a, o, i, e.startSourceSpan, e.sourceSpan); n.create.push(c), px(n, c, e, l), Pf(c, e), sn(t, e.children); for (let { name: p, value: h } of e.variables)
    t.contextVariables.set(p, h !== "" ? h : "$implicit"); if (l === Ve.NgTemplate && e.i18n instanceof ke) {
...
function ex(n, e) { if (e.i18n !== void 0 && !(e.i18n instanceof Ot))
    throw Error(`Unhandled i18n metadata type for element: ${e.i18n.constructor.name}`); let t = null; e.children.some(i => !(i instanceof ea) && (!(i instanceof On) || i.value.trim().length > 0)) && (t = n.job.allocateView(n.xref), sn(t, e.children)); let s = n.job.allocateXrefId(), r = Mg(s, e.selector, e.i18n, t?.xref ?? null, e.sourceSpan); for (let i of e.attributes) {
...
function If(n, e, t) { let s = e.value; if (s instanceof Ge && (s = s.ast), !(s instanceof hi))
    throw new Error(`AssertionError: expected Interpolation for BoundText node, got ${s.constructor.name}`); if (e.i18n !== void 0 && !(e.i18n instanceof Ke))
    throw Error(`Unhandled i18n metadata type for text interpolation: ${e.i18n?.constructor.name}`); let r = e.i18n instanceof Ke ? e.i18n.children.filter(a => a instanceof ht).map(a => a.name) : []; if (r.length > 0 && r.length !== s.expressions.length)
    throw Error(`Unexpected number of i18n placeholders (${s.expressions.length}) for BoundText with ${s.expressions.length} expressions`); let i = n.job.allocateXrefId(); n.create.push(Zh(i, "", t, e.sourceSpan)); let o = n.job.compatibility ? null : e.sourceSpan; n.update.push(dg(i, new ce(s.strings, s.expressions.map(a => q(a, n.job, o)), r), e.sourceSpan)); }
function tx(n, e) { let t = null, s = []; for (let r = 0; r < e.branches.length; r++) {
...
function nx(n, e) { if (e.cases.length === 0)
...
function _i(n, e, t, s, r) { if (t !== void 0 && !(t instanceof Vt))
    throw Error("Unhandled i18n metadata type for defer block"); if (s === void 0)
    return null; let i = n.job.allocateView(n.xref); sn(i, s); let o = Xh(i.xref, Ve.Block, null, `Defer${e}`, be.HTML, t, r, r); return n.create.push(o), o; }
function sx(n, e) { let t = null; if (n.job.deferMeta.mode === 0) {
...
function ix(n, e) { if (e.i18n instanceof ke && z2(e.i18n)) {
    let t = n.job.allocateXrefId();
...
function ox(n, e) { let t = n.job.allocateView(n.xref), s = `\u0275$index_${t.xref}`, r = `\u0275$count_${t.xref}`, i = new Set; t.contextVariables.set(e.item.name, e.item.value); for (let _ of e.contextVariables)
    _.value === "$index" && i.add(_.name), _.name === "$index" ? t.contextVariables.set("$index", _.value).set(s, _.value) : _.name === "$count" ? t.contextVariables.set("$count", _.value).set(r, _.value) : t.aliases.add({ kind: Se.Alias, name: null, identifier: _.name, expression: ax(_, s, r) }); let o = me(e.trackBy.span, e.sourceSpan), a = q(e.trackBy, n.job, o); sn(t, e.children); let l = null, c = null; e.empty !== null && (l = n.job.allocateView(n.xref), sn(l, e.empty.children), c = $o(n, l.xref, e.empty)); let p = { $index: i, $implicit: e.item.name }; if (e.i18n !== void 0 && !(e.i18n instanceof Vt))
    throw Error("AssertionError: Unhandled i18n metadata type or @for"); if (e.empty?.i18n !== void 0 && !(e.empty.i18n instanceof Vt))
    throw Error("AssertionError: Unhandled i18n metadata type or @empty"); let h = e.i18n, m = e.empty?.i18n, v = $o(n, t.xref, e), w = Ig(t.xref, l?.xref ?? null, v, a, p, c, h, m, e.startSourceSpan, e.sourceSpan); n.create.push(w); let C = q(e.expression, n.job, me(e.expression.span, e.sourceSpan)), b = Cg(w.xref, w.handle, C, e.sourceSpan); n.update.push(b); }
function ax(n, e, t) { switch (n.value) {
...
function q(n, e, t) { if (n instanceof Ge)
    return q(n.ast, e, t); if (n instanceof mn)
    return n.receiver instanceof Rt && !(n.receiver instanceof Kn) ? new Ce(n.name) : new Ue(q(n.receiver, e, t), n.name, null, me(n.span, t)); if (n instanceof Dr) {
...
function Xt(n) { if (n == null)
    return null; if (!(n instanceof ke))
    throw Error(`Expected i18n meta to be a Message, but got: ${n.constructor.name}`); return n; }
function ux(n, e, t) { let s = new Array, r = new Set; for (let i of t.attributes) {
...
function px(n, e, t, s) { let r = new Array; for (let i of t.templateAttrs)
    if (i instanceof Ms) {
        let o = zn.securityContext(Gn, i.name, !0);
        r.push(Ti(n, e.xref, Y.Attribute, i.name, i.value, null, o, !0, s, Xt(i.i18n), i.sourceSpan));
    }
    else
        r.push(Ti(n, e.xref, i.type, i.name, ai(i.value), i.unit, i.securityContext, !0, s, Xt(i.i18n), i.sourceSpan)); for (let i of t.attributes) {
    let o = zn.securityContext(Gn, i.name, !0);
    r.push(Ti(n, e.xref, Y.Attribute, i.name, i.value, null, o, !1, s, Xt(i.i18n), i.sourceSpan));
} for (let i of t.inputs)
    r.push(Ti(n, e.xref, i.type, i.name, ai(i.value), i.unit, i.securityContext, !1, s, Xt(i.i18n), i.sourceSpan)); n.create.push(r.filter(i => i?.kind === u.ExtractedAttribute)), n.update.push(r.filter(i => i?.kind === u.Binding)); for (let i of t.outputs) {
    if (i.type === pt.LegacyAnimation && i.phase === null)
        throw Error("Animation listener should have a phase");
...
function Ti(n, e, t, s, r, i, o, a, l, c, p) { let h = typeof r == "string"; if (l === Ve.Structural) {
    if (!a)
        switch (t) {
            case Y.Property:
            case Y.Class:
    ...
    if (!h && (t === Y.Attribute || t === Y.LegacyAnimation))
        return null;
} let m = Nf.get(t); return l === Ve.NgTemplate && (t === Y.Class || t === Y.Style || t === Y.Attribute && !h) && (m = I.Property), ss(e, m, s, ql(n.job, r, c), i, o, h, a, l, c, p); }
function Hc(n, e, t) { e = ai(e); let s = new Array, r = e instanceof Ps ? e.expressions : [e]; if (r.length === 0)
    throw new Error("Expected listener to have non-empty expression list."); let i = r.map(a => q(a, n.job, t)), o = i.pop(); return s.push(...i.map(a => It(new tt(a, a.sourceSpan)))), s.push(It(new _e(o, o.sourceSpan))), s; }
function Df(n, e, t) { e = ai(e); let s = new Array; if (e instanceof Ps)
    if (e.expressions.length === 1)
        e = e.expressions[0];
    else
        throw new Error("Expected two-way listener to have a single expression."); let r = q(e, n.job, t), i = new Ce("$event"), o = new Eo(r, i); return s.push(It(new tt(o))), s.push(It(new _e(i))), s; }
function ai(n) { return n instanceof Ge ? n.ast : n; }
function Pf(n, e) { hx(n.localRefs); for (let { name: t, value: s } of e.references)
    n.localRefs.push({ name: t, target: s }); }
function hx(n) { if (!Array.isArray(n))
...
function $o(n, e, t) { let s = null; for (let r of t.children)
...
function mx(n, e) { if (Array.isArray(n.predicate)) {
    let t = [];
...
function Mf(n) { let e = [], t = 0, s = () => { t > 0 && (e.unshift(y(f.queryAdvance).callFn(t === 1 ? [] : [d(t)]).toStmt()), t = 0); }; for (let r = n.length - 1; r >= 0; r--) {
    let i = n[r];
    i === Wc ? t++ : (s(), e.unshift(i));
} return s(), e; }
function gx(n, e, t) { let s = [], r = [], i = Eh(a => r.push(a), Ic); n.forEach(a => { let l = Bf(a, e, { signalBased: f.viewQuerySignal, nonSignal: f.viewQuery }); if (s.push(l.toStmt()), a.isSignal) {
    r.push(Wc);
    return;
} let c = i(), p = y(f.loadQuery).callFn([]), h = y(f.queryRefresh).callFn([c.set(p)]), m = N(Ur).prop(a.propertyName).set(a.first ? c.prop("first") : c); r.push(h.and(m).toStmt()); }); let o = t ? `${t}_Query` : null; return en([new Z(Nc, Zo), new Z(Ur, null)], [Oo(1, s), Oo(2, Mf(r))], Oe, null, o); }
function vx(n, e, t) { let s = [], r = [], i = Eh(a => r.push(a), Ic); for (let a of n) {
...
} let o = t ? `${t}_ContentQueries` : null; return en([new Z(Nc, Zo), new Z(Ur, null), new Z("dirIndex", null)], [Oo(1, s), Oo(2, Mf(r))], Oe, null, o); }
var Vo = class extends af {
    ...
    parseInlineTemplateBinding(e, t, s, r, i, o, a, l) { let c = s.start.offset + xx.length, p = this._parseTemplateBindings(e, t, s, c, r); for (let h of p) {
    ...
    createBoundElementProperty(e, t, s = !1, r = !0) { if (t.isLegacyAnimation)
        return new Ki(t.name, Y.LegacyAnimation, te.NONE, t.expression, null, t.sourceSpan, t.keySpan, t.valueSpan); let i = null, o, a = null, l = t.name.split(wp), c; if (l.length > 1)
        if (l[0] == wx) {
            ...
            l[0] == Ex ? (a = l[1], o = Y.Class, c = [te.NONE]) : l[0] == Sx && (i = l.length > 2 ? l[2] : null, a = l[1], o = Y.Style, c = [te.STYLE]); if (a === null) {
...
function yx(n) { if (n == null || n.length === 0 || n[0] == "/")
    return !1; let e = n.match(Cx); return e === null || e[1] == "package" || e[1] == "asset"; }
...
function Ox(n, e, t, s) { let r = Wx(e), i = [], o = yp(n, r, s); o !== null && i.push(new Hn(o.expression, L(t, n.children, n.children), o.expressionAlias, n.sourceSpan, n.startSourceSpan, n.endSourceSpan, n.nameSpan, n.i18n)); for (let h of e)
    if (jc.test(h.name)) {
        let m = yp(h, r, s);
        if (m !== null) {
    ...
    } let a = i.length > 0 ? i[0].startSourceSpan : n.startSourceSpan, l = i.length > 0 ? i[i.length - 1].endSourceSpan : n.endSourceSpan, c = n.sourceSpan, p = i[i.length - 1]; return p !== void 0 && (c = new B(a.start, p.sourceSpan.end)), { node: new io(i, c, n.startSourceSpan, l, n.nameSpan), errors: r }; }
function Vx(n, e, t, s) { let r = [], i = Ux(n, r, s), o = null, a = null; for (let l of e)
    l.name === "empty" ? a !== null ? r.push(new D(l.sourceSpan, "@for loop can only have one @empty block")) : l.parameters.length > 0 ? r.push(new D(l.sourceSpan, "@empty block cannot have parameters")) : a = new Or(L(t, l.children, l.children), l.sourceSpan, l.startSourceSpan, l.endSourceSpan, l.nameSpan, l.i18n) : r.push(new D(l.sourceSpan, `Unrecognized @for loop block "${l.name}"`)); if (i !== null)
    if (i.trackBy === null)
...
function qx(n, e, t) { let s = jx(n), r = n.parameters.length > 0 ? li(n.parameters[0], t) : t.parseBinding("", !1, n.sourceSpan, 0), i = [], o = [], a = null; for (let l of n.children) {
    if (!(l instanceof it))
        continue;
...
function Ux(n, e, t) { if (n.parameters.length === 0)
    return e.push(new D(n.startSourceSpan, "@for loop does not have an expression")), null; let [s, ...r] = n.parameters, i = zx(s, e)?.match(Bx); if (!i || i[2].trim().length === 0)
    return e.push(new D(s.sourceSpan, 'Cannot parse expression. @for loop expression must match the pattern "<identifier> of <expression>"')), null; let [, o, a] = i; vr.has(o) && e.push(new D(s.sourceSpan, `@for loop item name cannot be one of ${Array.from(vr).join(", ")}.`)); let l = s.expression.split(" ")[0], c = new B(s.sourceSpan.start, s.sourceSpan.start.moveBy(l.length)), p = { itemName: new Cn(o, "$implicit", c, c), trackBy: null, expression: li(s, t, a), context: Array.from(vr, h => { let m = new B(n.startSourceSpan.end, n.startSourceSpan.end); return new Cn(h, h, m, m); }) }; for (let h of r) {
...
function Hx(n, e, t, s, r, i) { let o = e.split(","), a = t.start; for (let l of o) {
...
function Wx(n) { let e = [], t = !1; for (let s = 0; s < n.length; s++) {
    let r = n[s];
...
function jx(n) { let e = [], t = !1; if (n.parameters.length !== 1)
    return e.push(new D(n.startSourceSpan, "@switch block must have exactly one parameter")), e; for (let s of n.children)
...
function yp(n, e, t) { if (n.parameters.length === 0)
    return e.push(new D(n.startSourceSpan, "Conditional block does not have an expression")), null; let s = li(n.parameters[0], t), r = null; for (let i = 1; i < n.parameters.length; i++) {
...
function zx(n, e) { let t = n.expression, s = /^\s$/, r = 0, i = 0, o = t.length - 1; for (let a = 0; a < t.length; a++) {
    let l = t[a];
    if (l === "(")
        i = a + 1, r++;
    else {
...
function Ca({ expression: n, sourceSpan: e }, t, s, r) { let i = n.indexOf("when"), o = new B(e.start.moveBy(i), e.start.moveBy(i + 4)), a = zc(n, e), l = Gc(n, e); if (i === -1)
...
function Aa({ expression: n, sourceSpan: e }, t, s, r) { let i = n.indexOf("on"), o = new B(e.start.moveBy(i), e.start.moveBy(i + 2)), a = zc(n, e), l = Gc(n, e); if (i === -1)
...
var Wl = class {
    ...
    consumeTrigger(e, t) { let s = this.span.start.moveBy(this.start + e.index - this.tokens[0].index), r = new B(s, s.moveBy(e.strValue.length)), i = s.moveBy(this.token().end - e.index), o = e.index === 0, a = o ? this.onSourceSpan : null, l = o ? this.prefetchSpan : null, c = o ? this.hydrateSpan : null, p = new B(o ? this.span.start : s, i); try {
        switch (e.toString()) {
            case $e.IDLE:
    ...
    consumeParameters() { let e = []; if (!this.token().isCharacter(yt))
...
function Qx(n, e, t, s, r, i) { if (n.length > 0)
    throw new Error(`"${$e.IDLE}" trigger cannot have parameters`); return new Ja(e, t, s, r, i); }
function Zx(n, e, t, s, r, i) { if (n.length !== 1)
    throw new Error(`"${$e.TIMER}" trigger must have exactly one parameter`); let o = qo(n[0]); if (o === null)
    throw new Error(`Could not parse time value of trigger "${$e.TIMER}"`); return new el(o, e, t, s, r, i); }
function Jx(n, e, t, s, r, i) { if (n.length > 0)
    throw new Error(`"${$e.IMMEDIATE}" trigger cannot have parameters`); return new Ka(e, t, s, r, i); }
...
function ny(n, e) { if (e.length > 1)
    throw new Error(`"${n}" trigger can only have zero or one parameters`); }
function sy(n, e) { if (e.length > 0)
    throw new Error(`Hydration trigger "${n}" cannot have parameters`); }
function ci(n, e = 0) { let t = !1; for (let s = e; s < n.length; s++)
    if (Xx.test(n[s]))
        t = !0;
    else if (t)
        return s; return -1; }
function qo(n) { let e = n.match(Gx); if (!e)
...
function hy(n, e, t, s) { let r = [], { placeholder: i, loading: o, error: a } = fy(e, r, t), { triggers: l, prefetchTriggers: c, hydrateTriggers: p } = vy(n, s, r, i), h = n.endSourceSpan, m = n.sourceSpan.end; if (e.length > 0) {
    let C = e[e.length - 1];
    h = C.endSourceSpan, m = C.sourceSpan.end;
} let v = new B(n.sourceSpan.start, m); return { node: new es(L(t, n.children, n.children), l, c, p, i, o, a, n.nameSpan, v, n.sourceSpan, n.startSourceSpan, h, n.i18n), errors: r }; }
function fy(n, e, t) { let s = null, r = null, i = null; for (let o of n)
    try {
        if (!jl(o.name)) {
...
function dy(n, e) { let t = null; for (let s of n.parameters)
    if (Ff.test(s.expression)) {
        ...
        throw new Error(`Unrecognized parameter in @placeholder block: "${s.expression}"`); return new Mr(L(e, n.children, n.children), t, n.nameSpan, n.sourceSpan, n.startSourceSpan, n.endSourceSpan, n.i18n); }
function my(n, e) { let t = null, s = null; for (let r of n.parameters)
    if (cy.test(r.expression)) {
        ...
        throw new Error(`Unrecognized parameter in @loading block: "${r.expression}"`); return new Rr(L(e, n.children, n.children), t, s, n.nameSpan, n.sourceSpan, n.startSourceSpan, n.endSourceSpan, n.i18n); }
function gy(n, e) { if (n.parameters.length > 0)
    throw new Error("@error block cannot have parameters"); return new Fr(L(e, n.children, n.children), n.nameSpan, n.sourceSpan, n.startSourceSpan, n.endSourceSpan, n.i18n); }
function vy(n, e, t, s) { let r = {}, i = {}, o = {}; for (let a of n.parameters)
    uy.test(a.expression) ? Ca(a, e, r, t) : py.test(a.expression) ? Aa(a, r, t, s) : ry.test(a.expression) ? Ca(a, e, i, t) : iy.test(a.expression) ? Aa(a, i, t, s) : oy.test(a.expression) ? Ca(a, e, o, t) : ay.test(a.expression) ? Aa(a, o, t, s) : ly.test(a.expression) ? Yx(a, o, t) : t.push(new D(a.sourceSpan, "Unrecognized trigger")); return o.never && Object.keys(o).length > 1 && t.push(new D(n.startSourceSpan, "Cannot specify additional `hydrate` triggers if `hydrate never` is present")), { triggers: r, prefetchTriggers: i, hydrateTriggers: o }; }
...
var zl = class {
    ...
    visitElement(e) { let t = Li(e.i18n); t && (this.inI18nBlock && this.reportError("Cannot mark an element as translatable inside of a translatable section. Please remove the nested i18n marker.", e.sourceSpan), this.inI18nBlock = !0); let s = Rf(e); if (s.type === Ye.SCRIPT)
        return null; if (s.type === Ye.STYLE) {
        let _ = Cy(e);
        return _ !== null && this.styles.push(_), null;
    }
    else if (s.type === Ye.STYLESHEET && yx(s.hrefAttr))
        return this.styleUrls.push(s.hrefAttr), null; let r = sm(e.name), { attributes: i, boundEvents: o, references: a, variables: l, templateVariables: c, elementHasInlineTemplate: p, parsedProperties: h, templateParsedProperties: m, i18nAttrsMeta: v } = this.prepareAttributes(e.attrs, r), w = this.extractDirectives(e), C; s.nonBindable ? C = L(Ip, e.children).flat(1 / 0) : C = L(this, e.children, e.children); let b; if (s.type === Ye.NG_CONTENT) {
    ...
    visitExpansion(e) { if (!e.i18n)
        return null; if (!Li(e.i18n))
        throw new Error(`Invalid type "${e.i18n.constructor}" for "i18n" property of ${e.sourceSpan.toString()}. Expected a "Message"`); let t = e.i18n, s = {}, r = {}; return Object.keys(t.placeholders).forEach(i => { let o = t.placeholders[i]; if (i.startsWith(dm)) {
        let a = i.trim(), l = this.bindingParser.parseInterpolationExpression(o.text, o.sourceSpan);
        s[a] = new Bs(l, o.sourceSpan);
    }
    else
        r[i] = this._visitTextWithInterpolation(o.text, o.sourceSpan, null); }), new dh(s, r, e.sourceSpan, t); }
    ...
    visitComponent(e) { let t = Li(e.i18n); if (t && (this.inI18nBlock && this.reportError("Cannot mark a component as translatable inside of a translatable section. Please remove the nested i18n marker.", e.sourceSpan), this.inI18nBlock = !0), e.tagName !== null && Sy.has(e.tagName))
        return this.reportError(`Tag name "${e.tagName}" cannot be used as a component tag`, e.startSourceSpan), null; let { attributes: s, boundEvents: r, references: i, templateVariables: o, elementHasInlineTemplate: a, parsedProperties: l, templateParsedProperties: c, i18nAttrsMeta: p } = this.prepareAttributes(e.attrs, !1); this.validateSelectorlessReferences(i); let h = this.extractDirectives(e), m; e.attrs.find(C => C.name === "ngNonBindable") ? m = L(Ip, e.children).flat(1 / 0) : m = L(this, e.children, e.children); let v = this.categorizePropertyAttributes(e.tagName, l, p), w = new pr(e.componentName, e.tagName, e.fullName, s, v.bound, r, h, m, i, e.isSelfClosing, e.sourceSpan, e.startSourceSpan, e.endSourceSpan, e.i18n); return a && (w = this.wrapInTemplate(w, c, o, p, !1, t)), t && (this.inI18nBlock = !1), w; }
    ...
    prepareAttributes(e, t) { let s = [], r = [], i = [], o = [], a = [], l = {}, c = [], p = [], h = !1; for (let m of e) {
    ...
    parseAttribute(e, t, s, r, i, o, a) { let l = Np(t.name), c = t.value, p = t.sourceSpan, h = t.valueSpan ? t.valueSpan.start.offset : p.start.offset; function m(_, P, K) { let ne = t.name.length - l.length, de = _.start.moveBy(P.length + ne), oa = de.moveBy(K.length); return new B(de, oa, de, K); } let v = l.match(wy); if (v) {
        if (v[Ap] != null) {
        ...
        return !0;
    } let w = null; if (l.startsWith(on.BANANA_BOX.start) ? w = on.BANANA_BOX : l.startsWith(on.PROPERTY.start) ? w = on.PROPERTY : l.startsWith(on.EVENT.start) && (w = on.EVENT), w !== null && l.endsWith(w.end) && l.length > w.start.length + w.end.length) {
        ...
        return !0;
    } let C = m(p, "", l); return this.bindingParser.parsePropertyInterpolation(l, c, p, t.valueSpan, s, r, C, t.valueTokens ?? null); }
    extractDirectives(e) { let t = e instanceof ye ? e.tagName : e.name, s = [], r = new Set; for (let i of e.directives) {
        let o = !1;
    ...
    visitElement(e) { let t = Rf(e); if (t.type === Ye.SCRIPT || t.type === Ye.STYLE || t.type === Ye.STYLESHEET)
        return null; let s = L(this, e.children, null); return new Ft(e.name, L(this, e.attrs), [], [], [], s, [], e.isSelfClosing, e.sourceSpan, e.startSourceSpan, e.endSourceSpan); }
...
var Ay = [" ", `
`, "\r", "	"];
function $f(n, e, t = {}) { let { interpolationConfig: s, preserveWhitespaces: r, enableI18nLegacyMessageIdFormat: i } = t, o = t.enableSelectorless ?? !1, a = Uo(s, o), c = new Vo().parse(n, e, ue(E({ leadingTriviaChars: Ay }, t), { tokenizeExpansionForms: !0, tokenizeBlocks: t.enableBlockSyntax ?? !0, tokenizeLet: t.enableLetSyntax ?? !0, selectorlessEnabled: o })); if (!t.alwaysAttemptHtmlToR3AstConversion && c.errors && c.errors.length > 0) {
...
} let p = c.rootNodes, h = !(t.preserveSignificantWhitespace ?? !0), m = new Po(s, !r, i, void 0, t.preserveSignificantWhitespace, h), v = m.visitAllWithErrors(p); if (!t.alwaysAttemptHtmlToR3AstConversion && v.errors && v.errors.length > 0) {
    let de = { interpolationConfig: s, preserveWhitespaces: r, errors: v.errors, nodes: [], styleUrls: [], styles: [], ngContentSelectors: [] };
    return t.collectCommentNodes && (de.commentNodes = []), de;
} p = v.rootNodes, r || (p = L(new ii(!0, void 0, !1), p), m.hasI18nMeta && (p = L(new Po(s, !1, void 0, void 0, !0, h), p))); let { nodes: w, errors: C, styleUrls: b, styles: _, ngContentSelectors: P, commentNodes: K } = yy(p, a, { collectCommentNodes: !!t.collectCommentNodes }); C.push(...c.errors, ...v.errors); let ne = { interpolationConfig: s, preserveWhitespaces: r, errors: C.length > 0 ? C : null, nodes: w, styleUrls: b, styles: _, ngContentSelectors: P }; return t.collectCommentNodes && (ne.commentNodes = K), ne; }
var _y = new zs;
...
function by(n, e, t) { let s = Of(n, e, t); Vf(s, n); let r = n.selector && Xn.parse(n.selector), i = r && r[0]; if (i) {
    let w = i.getAttrs();
...
} n.encapsulation === null && (n.encapsulation = Et.Emulated); let h = !!n.externalStyles?.length; if (n.styles && n.styles.length) {
...
var Py = /^(?:\[([^\]]+)\])|(?:\(([^\)]+)\))$/;
function Ly(n) { let e = {}, t = {}, s = {}, r = {}; for (let i of Object.keys(n)) {
...
function Ry(n) { let e = [], t = !1; for (let s of n) {
...
function Wo(n) { let e = []; for (let t in n)
...
function bA(n, e) { let t = new wr; for (let c of e) {
...
} let s = $f(n, ""), i = new jo(t).bind({ template: s.nodes }), o = i.getEagerlyUsedDirectives().map(c => c.selector), a = i.getUsedDirectives().map(c => c.selector), l = i.getEagerlyUsedPipes(); return { directives: { regular: o, deferCandidates: Dp(a, o) }, pipes: { regular: l, deferCandidates: Dp(i.getUsedPipes(), l) } }; }
var jo = class {
    directiveMatcher;
    constructor(e) { this.directiveMatcher = e; }
    bind(e) { if (!e.template && !e.host)
        throw new Error("Empty bound targets are not supported"); let t = new Map, s = [], r = new Set, i = new Map, o = new Map, a = new Map, l = new Map, c = new Map, p = new Map, h = new Set, m = new Set, v = []; if (e.template) {
    ...
    visitComponent(e) { if (this.directiveMatcher instanceof Bi) {
    ...
    visitDirective(e) { if (this.directiveMatcher instanceof Bi) {
    ...
    visitElementOrTemplate(e) { if (this.directiveMatcher instanceof wr) {
        let t = [], s = gm(e);
    ...
    trackSelectorBasedBindingsAndDirectives(e, t) { this.trackMatchedDirectives(e, t), e.references.forEach(r => { let i = null; if (r.value.trim() === "")
        ...
        return; i !== null ? this.references.set(r, { directive: i, node: e }) : this.references.set(r, e); }); let s = (r, i) => { let o = t.find(l => l[i].hasBindingPropertyName(r.name)), a = o !== void 0 ? o : e; this.bindings.set(r, a); }; e.inputs.forEach(r => s(r, "inputs")), e.attributes.forEach(r => s(r, "inputs")), e instanceof Je && e.templateAttrs.forEach(r => s(r, "inputs")), e.outputs.forEach(r => s(r, "outputs")); }
    visitVariable(e) { }
    visitReference(e) { }
    visitTextAttribute(e) { }
    visitBoundAttribute(e) { }
    ...
    maybeMap(e, t) { if (!(e.receiver instanceof Rt) || e.receiver instanceof Kn)
    ...
    getDeferredTriggerTarget(e, t) { if (!(t instanceof no) && !(t instanceof so) && !(t instanceof to))
        ...
        return i;
    } let r = this.findEntityInScope(e, s); if (r instanceof Vr && this.getDefinitionNodeOfSymbol(r) !== e) {
...
function Fy(n, e) { let t = new Map; function s(i) { if (t.has(i.rootNode))
...
function Bp(n) { let e = Ky(n.inputs || []), t = ka(n.outputs || []), s = n.propMetadata, r = {}, i = {}; for (let a in s)
    s.hasOwnProperty(a) && s[a].forEach(l => { Yy(l) ? r[a] = { bindingPropertyName: l.alias || a, classPropertyName: a, required: l.required || !1, isSignal: !!l.isSignal, transformFunction: l.transform != null ? new U(l.transform) : null } : Qy(l) && (i[a] = l.alias || a); }); let o = n.hostDirectives?.length ? n.hostDirectives.map(a => typeof a == "function" ? { directive: ge(a), inputs: null, outputs: null, isForwardReference: !1 } : { directive: ge(a.directive), isForwardReference: !1, inputs: a.inputs ? ka(a.inputs) : null, outputs: a.outputs ? ka(a.outputs) : null }) : null; return ue(E({}, n), { typeArgumentCount: 0, typeSourceSpan: n.typeSourceSpan, type: ge(n.type), deps: null, host: E({}, zy(n.propMetadata, n.typeSourceSpan, n.host)), inputs: E(E({}, e), r), outputs: E(E({}, t), i), queries: n.queries.map(Pp), providers: n.providers != null ? new U(n.providers) : null, viewQueries: n.viewQueries.map(Pp), fullInheritance: !1, hostDirectives: o }); }
...
function Mp(n) { let e = null; for (let t = 1; t < n.length; t += 2)
    e = e || {}, e[n[t - 1]] = n[t]; return e; }
function Oy(n) { let e = {}; for (let t of Object.keys(n))
    e[t] = new U(n[t]); return e; }
function Vy(n, e, t) { let { template: s, interpolation: r, defer: i } = Gf(n.template, n.type.name, t, n.preserveWhitespaces ?? !1, n.interpolation, n.deferBlockDependencies), o = []; if (n.dependencies)
...
function Gf(n, e, t, s, r, i) { let o = r ? co.fromArray(r) : ot, a = $f(n, t, { preserveWhitespaces: s, interpolationConfig: o }); if (a.errors !== null) {
    let p = a.errors.map(h => h.toString()).join(", ");
    throw new Error(`Errors during JIT compilation of template for ${e}: ${p}`);
} let c = new jo(null).bind({ template: a.nodes }); return { template: a, interpolation: o, defer: jy(c, i) }; }
function ds(n, e) { if (n.hasOwnProperty(e))
    return yc(new U(n[e]), 0); }
function Rp(n, e) { if (n.hasOwnProperty(e))
    return new U(n[e]); }
...
function jy(n, e) { let t = n.getDeferBlocks(), s = new Map; for (let r = 0; r < t.length; r++) {
    let i = e?.[r];
...
function Ky(n) { return n.reduce((e, t) => { if (typeof t == "string") {
    let [s, r] = Qf(t);
    e[r] = { bindingPropertyName: s, classPropertyName: r, required: !1, isSignal: !1, transformFunction: null };
}
else
    e[t.name] = { bindingPropertyName: t.alias || t.name, classPropertyName: t.name, required: t.required || !1, isSignal: !1, transformFunction: t.transform != null ? new U(t.transform) : null }; return e; }, {}); }
...
var sc = class {
    ...
    visitExpansionCase(e, t) { let s = L(this, e.expression, t); if (this._mode === Dt.Merge)
    ...
    visitComment(e, t) { let s = lC(e); if (s && this._isInTranslatableSection) {
        this._reportError(e, "Could not start a block inside a translatable section");
        return;
    } let r = cC(e); if (r && !this._inI18nBlock) {
    ...
    _visitElementLike(e, t) { this._mayBeAddBlockChildren(e), this._depth++; let s = this._inI18nNode, r = this._inImplicitNode, i = [], o, a = e instanceof ye ? e.tagName : e.name, l = uC(e), c = l ? l.value : "", p = this._implicitTags.some(m => a === m) && !this._inIcu && !this._isInTranslatableSection, h = !r && p; if (this._inImplicitNode = r || p, !this._isInTranslatableSection && !this._inIcu) {
        if (l || h) {
            this._inI18nNode = !0;
    ...
    _addMessage(e, t) { if (e.length == 0 || this._isEmptyAttributeValue(e) || this._isPlaceholderOnlyAttributeValue(e) || this._isPlaceholderOnlyMessage(e))
        return null; let { meaning: s, description: r, id: i } = qp(t), o = this._createI18nMessage(e, s, r, i); return this._messages.push(o), o; }
    _isEmptyAttributeValue(e) { return Up(e) ? e[0].value.trim() === "" : !1; }
    _isPlaceholderOnlyAttributeValue(e) { if (!Up(e))
    ...
    _isPlaceholderOnlyMessage(e) { if (!pC(e))
    ...
    _translateAttributes(e) { let t = {}, s = []; return e.attrs.forEach(r => { r.name.startsWith(sr) && (t[r.name.slice(sr.length)] = qp(r.value)); }), e.attrs.forEach(r => { if (!(r.name === Zf || r.name.startsWith(sr)))
    ...
    _closeTranslatableSection(e, t) { if (!this._isInTranslatableSection) {
        ...
        return;
    } let s = this._msgCountAtSectionStart; if (t.reduce((i, o) => i + (o instanceof In ? 0 : 1), 0) == 1)
...
function qp(n) { if (!n)
    return { meaning: "", description: "", id: "" }; let e = n.indexOf(iC), t = n.indexOf(rC), [s, r] = e > -1 ? [n.slice(0, e), n.slice(e + 2)] : [n, ""], [i, o] = t > -1 ? [s.slice(0, t), s.slice(t + 1)] : ["", s]; return { meaning: i, description: o, id: r.trim() }; }
...
`)}`);
    ...
    visitContainer(e, t) { let s = []; return e.children.forEach(r => s.push(...r.visit(this))), s; }
    visitIcu(e, t) { let s = [new X(`{${e.expressionPlaceholder}, ${e.type}, `)]; return Object.keys(e.cases).forEach(r => { s.push(new X(`${r} {`), ...e.cases[r].visit(this), new X("} ")); }), s.push(new X("}")), s; }
    visitTagPlaceholder(e, t) { let s = _C(e.tag); if (e.isVoid)
        return [new M(Wt, { id: e.startName, ctype: s, "equiv-text": `<${e.tag}/>` })]; let r = new M(Wt, { id: e.startName, ctype: s, "equiv-text": `<${e.tag}>` }), i = new M(Wt, { id: e.closeName, ctype: s, "equiv-text": `</${e.tag}>` }); return [r, ...this.serialize(e.children), i]; }
...
function _C(n) { switch (n.toLowerCase()) {
    case "br": return "lb";
...
`)}`);
    ...
    visitContainer(e, t) { let s = []; return e.children.forEach(r => s.push(...r.visit(this))), s; }
    visitIcu(e, t) { let s = [new X(`{${e.expressionPlaceholder}, ${e.type}, `)]; return Object.keys(e.cases).forEach(r => { s.push(new X(`${r} {`), ...e.cases[r].visit(this), new X("} ")); }), s.push(new X("}")), s; }
    visitTagPlaceholder(e, t) { let s = PC(e.tag); if (e.isVoid)
        return [new M(qn, { id: (this._nextPlaceholderId++).toString(), equiv: e.startName, type: s, disp: `<${e.tag}/>` })]; let r = new M(lc, { id: (this._nextPlaceholderId++).toString(), equivStart: e.startName, equivEnd: e.closeName, type: s, dispStart: `<${e.tag}>`, dispEnd: `</${e.tag}>` }), i = [].concat(...e.children.map(o => o.visit(this))); return i.length ? i.forEach(o => r.children.push(o)) : r.children.push(new X("")), [r]; }
...
function PC(n) { switch (n.toLowerCase()) {
    case "br":
    case "b":
    case "i":
    case "u": return "fmt";
...
`));
    ...
    getTagDefinition;
    _translationBundle;
    constructor(e, t, s, r = Ts.Warning, i) { if (this._htmlParser = e, t) {
        let o = BC(s);
...
function BC(n) { switch (n = (n || "xlf").toLowerCase(), n) {
...
var Xp = class {
    ...
    updateFromTemplate(e, t, s) { let r = this._htmlParser.parse(e, t, { tokenizeExpansionForms: !0, interpolationConfig: s }); if (r.errors.length)
        return r.errors; let i = this._preserveWhitespace ? r.rootNodes : gt(new ii(!1), r.rootNodes), o = oC(i, s, this._implicitTags, this._implicitAttrs, this._preserveWhitespace); return o.errors.length ? o.errors : (this._messages.push(...o.messages), []); }
...
function BA(n, e) { if (e === null || e.length === 0)
    return $C(n); let t = new oe, s = new oe; return s.set("decorators", n.decorators), s.set("ctorParameters", n.ctorParameters ?? d(null)), s.set("propDecorators", n.propDecorators ?? d(null)), t.set("minVersion", d(FC)), t.set("version", d("20.1.0")), t.set("ngImport", y(f.core)), t.set("type", n.type), t.set("resolveDeferredDeps", sd(e)), t.set("resolveMetadata", ie(e.map(r => new Z(r.symbolName, qe)), s.toLiteralMap())), y(f.declareClassMetadataAsync).callFn([t.toLiteralMap()]); }
...
function WC(n) { let e = Object.getOwnPropertyNames(n); return e.length === 0 ? null : pe(e.map(t => { let s = n[t], r = s.bindingPropertyName, i = r !== t, o; if (i || s.transformFunction !== null) {
    let a = [Le(r), Le(t)];
    s.transformFunction !== null && a.push(s.transformFunction), o = R(a);
}
else
    o = Le(r); return { key: t, quoted: kc.test(t), value: o }; })); }
...
function jC(n, e, t) { let s = id(n), r = new wc; if (j(r, e.nodes), s.set("template", zC(e, t)), t.isInline && s.set("isInline", d(!0)), r.hasBlocks && s.set("minVersion", d("17.0.0")), s.set("styles", cr(n.styles, d)), s.set("dependencies", XC(n)), s.set("viewProviders", n.viewProviders), s.set("animations", n.animations), n.changeDetection !== null) {
...
function zC(n, e) { if (e.inlineTemplateLiteralExpression !== null)
    return e.inlineTemplateLiteralExpression; if (e.isInline)
    return d(e.content, null, null); let t = e.content, s = new zr(t, e.sourceUrl), r = new ns(s, 0, 0, 0), i = GC(s, t), o = new B(r, i); return d(t, null, o); }
function GC(n, e) {
...
function XC(n) { let e = n.declarationListEmitMode !== 0 ? Cc : t => t; if (n.declarationListEmitMode === 3)
    throw new Error("Unsupported emit mode"); return cr(n.declarations, t => { switch (t.kind) {
    case Qt.Directive:
        let s = new oe;
...
function ZC(n) { let e = new oe; if (e.set("minVersion", d(QC)), e.set("version", d("20.1.0")), e.set("ngImport", y(f.core)), e.set("type", n.type.value), n.providedIn !== void 0) {
...
function tA(n) { let e = new oe; if (n.kind === Gt.Local)
    throw new Error("Invalid path! Local compilation mode should not get into the partial compilation path"); return e.set("minVersion", d(eA)), e.set("version", d("20.1.0")), e.set("ngImport", y(f.core)), e.set("type", n.type.value), n.bootstrap.length > 0 && e.set("bootstrap", xt(n.bootstrap, n.containsForwardDecls)), n.declarations.length > 0 && e.set("declarations", xt(n.declarations, n.containsForwardDecls)), n.imports.length > 0 && e.set("imports", xt(n.imports, n.containsForwardDecls)), n.exports.length > 0 && e.set("exports", xt(n.exports, n.containsForwardDecls)), n.schemas !== null && n.schemas.length > 0 && e.set("schemas", R(n.schemas.map(t => t.value))), n.id !== null && e.set("id", n.id), e; }
var nA = "14.0.0";
...
function WA(n, e, t) { let s = "\u0275\u0275namespaces", r = [t.className, s].map(o => new Z(o, qe)), i = []; for (let o of t.localDependencies)
    r.push(new Z(o.name)); for (let o = 0; o < t.namespaceDependencies.length; o++)
    i.push(new le(t.namespaceDependencies[o].assignedName, N(s).key(d(o)), qe, ae.Final)); i.push(...e); for (let o of n)
